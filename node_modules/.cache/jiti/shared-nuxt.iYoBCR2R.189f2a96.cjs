"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = loadNuxt;exports.b = build;exports.c = createNuxt;exports.l = void 0;var _nodeFs = _interopRequireWildcard(require("node:fs"));
...
var _unpluginVueRouter = require("unplugin-vue-router");
var _options2 = require("unplugin-vue-router/options");
...
var _stripLiteral = require("strip-literal");
var _vue = require("@unhead/vue");
var _globby = require("globby");
var _ultrahtml = require("ultrahtml");
var _unimport = require("unimport");
var _vueRouter = require("vue-router");
var _transform = require("unctx/transform");
var _nodeOs = require("node:os");
var _nitropack = require("nitropack");
var _h = require("h3");
...
var _nanotar = require("nanotar");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _getRequireWildcardCache(e) {if ("function" != typeof WeakMap) return null;var r = new WeakMap(),t = new WeakMap();return (_getRequireWildcardCache = function (e) {return e ? t : r;})(e);}function _interopRequireWildcard(e, r) {if (!r && e && e.__esModule) return e;if (null === e || "object" != typeof e && "function" != typeof e) return { default: e };var t = _getRequireWildcardCache(r);if (t && t.has(e)) return t.get(e);var n = { __proto__: null },a = Object.defineProperty && Object.getOwnPropertyDescriptor;for (var u in e) if ("default" !== u && {}.hasOwnProperty.call(e, u)) {var i = a ? Object.getOwnPropertyDescriptor(e, u) : null;i && (i.get || i.set) ? Object.defineProperty(n, u, i) : n[u] = e[u];}return n.default = e, t && t.set(e, n), n;}

let _distDir = (0, _pathe.dirname)((0, _nodeUrl.fileURLToPath)("file:///C:/Users/admin/Documents/Front_End_Web_Hotel_Nhom_2/node_modules/nuxt/dist/shared/nuxt.iYoBCR2R.mjs"));
if (_distDir.match(/(chunks|shared)$/)) {
  _distDir = (0, _pathe.dirname)(_distDir);
}
const distDir = _distDir;
const pkgDir = (0, _pathe.resolve)(distDir, "..");

async function resolveTypePath(path, subpath, searchPaths = (0, _kit.tryUseNuxt)()?.options.modulesDir) {
  try {
    const r = (0, _exsolve.resolveModulePath)(path, {
      from: searchPaths?.map((d) => (0, _kit.directoryToURL)(d)),
    ...
    const rootPath = await (0, _pkgTypes.resolvePackageJSON)(r);
    return (0, _pathe.dirname)(rootPath);
  } catch {
    return null;
  }
}
...
async function isDirectory$1(path) {
  return (await _nodeFs.promises.lstat(path)).isDirectory();
}
const logger = exports.l = (0, _kit.useLogger)("nuxt");

async function transform(input, options) {
  return await (0, _esbuild.transform)(input, { ...(0, _kit.tryUseNuxt)()?.options.esbuild.options, ...options });
}
function walk(ast, callback) {
  return (0, _estreeWalker.walk)(ast, {
...
async function initParser() {
  try {
    parseSync = await Promise.resolve().then(() => jitiImport('oxc-parser').then((m) => _interopRequireWildcard(m))).then((r) => r.parseSync);
  } catch {
    const { parseSync: wasmParse } = await Promise.resolve().then(() => jitiImport('@oxc-parser/wasm').then((m) => _interopRequireWildcard(m)));
...
function getNameFromPath(path, relativeTo) {
  const relativePath = relativeTo ? (0, _pathe.normalize)(path).replace((0, _ufo.withTrailingSlash)((0, _pathe.normalize)(relativeTo)), "") : (0, _pathe.basename)(path);
  const prefixParts = (0, _scule.splitByCase)((0, _pathe.dirname)(relativePath));
  const fileName = (0, _pathe.basename)(relativePath, (0, _pathe.extname)(relativePath));
  const segments = resolveComponentNameSegments(fileName.toLowerCase() === "index" ? "" : fileName, prefixParts).filter(Boolean);
  return (0, _scule.kebabCase)(segments).replace(QUOTE_RE, "");
}
function hasSuffix(path, suffix) {
  return (0, _pathe.basename)(path, (0, _pathe.extname)(path)).endsWith(suffix);
}
function resolveComponentNameSegments(fileName, prefixParts) {
  const fileNameParts = (0, _scule.splitByCase)(fileName);
  const fileNamePartsContent = fileNameParts.join("/").toLowerCase();
  const componentNameParts = prefixParts.flatMap((p) => (0, _scule.splitByCase)(p));
  let index = prefixParts.length - 1;
  const matchedSuffix = [];
  while (index >= 0) {
    const prefixPart = prefixParts[index];
    matchedSuffix.unshift(...(0, _scule.splitByCase)(prefixPart).map((p) => p.toLowerCase()));
...
function isVue(id, opts = {}) {
  const { search } = (0, _ufo.parseURL)(decodeURIComponent((0, _nodeUrl.pathToFileURL)(id).href));
  ...
  const query = (0, _ufo.parseQuery)(search);
...
function isJS(id) {
  const { pathname } = (0, _ufo.parseURL)(decodeURIComponent((0, _nodeUrl.pathToFileURL)(id).href));
  return JS_RE$1.test(pathname);
}
function getLoader(id) {
  const { pathname } = (0, _ufo.parseURL)(decodeURIComponent((0, _nodeUrl.pathToFileURL)(id).href));
  const ext = (0, _pathe.extname)(pathname);
  if (ext === ".vue") {
    return "vue";
  }
  if (!JS_RE$1.test(ext)) {
...
async function resolvePagesRoutes(pattern, nuxt = (0, _kit.useNuxt)()) {
  const pagesDirs = nuxt.options._layers.map(
    (layer) => (0, _pathe.resolve)(layer.config.srcDir, (layer.config.rootDir === nuxt.options.rootDir ? nuxt.options : layer.config).dir?.pages || "pages")
  );
  const scannedFiles = [];
  for (const dir of pagesDirs) {
    const files = await (0, _kit.resolveFiles)(dir, pattern);
    scannedFiles.push(...files.map((file) => ({ relativePath: (0, _pathe.relative)(dir, file), absolutePath: file })));
  }
...
function generateRoutesFromFiles(files, options = {}) {
  ...
  for (const file of sortedFiles) {
    const segments = file.relativePath.replace(new RegExp(`${(0, _escapeStringRegexp.default)((0, _pathe.extname)(file.relativePath))}$`), "").split("/");
    const route = {
      name: "",
      path: "",
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      const path = (0, _ufo.withLeadingSlash)((0, _ufo.joinURL)(route.path, routePath.replace(INDEX_PAGE_RE, "/")));
...
async function augmentPages(routes, vfs, ctx = {}) {
  ...
  for (const route of routes) {
    if (route.file && !ctx.pagesToSkip?.has(route.file)) {
      const fileContent = route.file in vfs ? vfs[route.file] : _nodeFs.default.readFileSync(ctx.fullyResolvedPaths?.has(route.file) ? route.file : await (0, _kit.resolvePath)(route.file), "utf-8");
...
function getRouteMeta(contents, absolutePath, extraExtractionKeys = []) {
  ...
  if (absolutePath in metaCache$1 && metaCache$1[absolutePath]) {
    return (0, _klona.klona)(metaCache$1[absolutePath]);
  }
  ...
  return (0, _klona.klona)(extractedMeta);
}
const COLON_RE = /:/g;
function getRoutePath(tokens, hasSucceedingSegment = false) {
  return tokens.reduce((path, token) => {
    return path + (token.type === 2 /* optional */ ? `:${token.value}?` : token.type === 1 /* dynamic */ ? `:${token.value}()` : token.type === 3 /* catchall */ ? hasSucceedingSegment ? `:${token.value}([^/]*)*` : `:${token.value}(.*)*` : token.type === 4 /* group */ ? "" : (0, _ufo.encodePath)(token.value).replace(COLON_RE, "\\:"));
  }, "/");
}
...
function normalizeRoutes(routes, metaImports = /* @__PURE__ */new Set(), options) {
  return {
    imports: metaImports,
    routes: (0, _knitwork.genArrayFromRaw)(routes.map((page) => {
      ...
      const file = (0, _pathe.normalize)(page.file);
      const pageImportName = (0, _knitwork.genSafeVariableName)((0, _utils2.filename)(file) + (0, _ohash.hash)(file));
      const metaImportName = pageImportName + "Meta";
      metaImports.add((0, _knitwork.genImport)(`${file}?macro=true`, [{ name: "default", as: metaImportName }]));
      if (page._sync) {
        metaImports.add((0, _knitwork.genImport)(file, [{ name: "default", as: pageImportName }]));
      }
      const pageImport = page._sync && page.mode !== "client" ? pageImportName : (0, _knitwork.genDynamicImport)(file);
      const metaRoute = {
...
function resolveRoutePaths(page, parent = "/") {
  return [
  (0, _ufo.joinURL)(parent, page.path),
  ...(page.children?.flatMap((child) => resolveRoutePaths(child, (0, _ufo.joinURL)(parent, page.path))) || [])];
...
function isSerializable(code, node) {
  ...
  if (propertyValue.type === "ObjectExpression") {
    ...
    try {
      return {
        value: JSON.parse((0, _nodeVm.runInNewContext)(`JSON.stringify(${valueString})`, {})),
        serializable: true
      };
    } catch {
      return {
  ...
  for (const script of contents) {
    ...
    parseAndWalk(code, "file." + (script?.loader || "ts"), (node) => {
      ...
      if (node.callee.name === "defineRouteRules") {
        ...
        try {
          rule = JSON.parse((0, _nodeVm.runInNewContext)(rulesString.replace("defineRouteRules", "JSON.stringify"), {}));
        } catch {
...
function getMappedPages(pages, paths = {}, prefix = "") {
  for (const page of pages) {
    if (page.file) {
      const filename = (0, _pathe.normalize)(page.file);
...
const PageMetaPlugin = (options = {}) => (0, _unplugin.createUnplugin)(() => {
  return {
    ...
    transform(code, id) {
      ...
      const imports = (0, _mlly.findStaticImports)(code);
      ...
      const currentExports = (0, _mlly.findExports)(code);
      ...
      for (const i of imports) {
        const parsed = (0, _mlly.parseStaticImport)(i);
        for (const name of [
      ...
      walk(ast, {
        ...
        enter: (node) => {
          ...
          const m = new _magicString.default(metaCode);
...
function parseMacroQuery(id) {
  const { search } = (0, _ufo.parseURL)(decodeURIComponent((0, _pathe.isAbsolute)(id) ? (0, _nodeUrl.pathToFileURL)(id).href : id).replace(/\?macro=true$/, ""));
  const query = (0, _ufo.parseQuery)(search);
...
const RouteInjectionPlugin = (nuxt) => (0, _unplugin.createUnplugin)(() => {
  return {
    ...
    transform(code) {
      ...
      let replaced = false;
      const s = new _magicString.default(code);
      const strippedCode = (0, _stripLiteral.stripLiteral)(code);
...
const runtimeDir = (0, _pathe.resolve)(distDir, "pages/runtime");
async function resolveRouterOptions(nuxt, builtInRouterOptions) {
  const context = {
    files: []
  };
  for (const layer of nuxt.options._layers) {
    const path = await (0, _kit.findPath)((0, _pathe.resolve)(layer.config.srcDir, layer.config.dir?.app || "app", "router.options"));
    if (path) {
...
const pagesModule = (0, _kit.defineNuxtModule)({
  meta: {
    name: "nuxt:pages",
    configKey: "pages"
  },
  ...
  async setup(_options, nuxt) {
    ...
    const builtInRouterOptions = (await (0, _kit.findPath)((0, _pathe.resolve)(runtimeDir, "router.options"))) || (0, _pathe.resolve)(runtimeDir, "router.options");
    const pagesDirs = nuxt.options._layers.map(
      (layer) => (0, _pathe.resolve)(layer.config.srcDir, (layer.config.rootDir === nuxt.options.rootDir ? nuxt.options : layer.config).dir?.pages || "pages")
    );
    ...
    const isNonEmptyDir = (dir) => (0, _nodeFs.existsSync)(dir) && (0, _nodeFs.readdirSync)(dir).length;
    ...
    if (nuxt.options.dev && options.enabled) {
      (0, _kit.addPlugin)((0, _pathe.resolve)(runtimeDir, "plugins/check-if-page-unused"));
    }
    ...
    const restartPaths = nuxt.options._layers.flatMap((layer) => {
      ...
      return [
      (0, _pathe.resolve)(layer.config.srcDir || layer.cwd, layer.config.dir?.app || "app", "router.options.ts"),
      (0, _pathe.resolve)(layer.config.srcDir || layer.cwd, pagesDir)];

    });
    nuxt.hooks.hook("builder:watch", async (event, relativePath) => {
      const path = (0, _pathe.resolve)(nuxt.options.srcDir, relativePath);
    ...
    if (!options.enabled) {
      (0, _kit.addPlugin)((0, _pathe.resolve)(distDir, "app/plugins/router"));
      (0, _kit.addTemplate)({
        filename: "pages.mjs",
        getContents: () => [
      ...
      (0, _kit.addComponent)({
        ...
        filePath: (0, _pathe.resolve)(distDir, "pages/runtime/page-placeholder")
      });
    ...
    if (useExperimentalTypedPages) {
      ...
      const typedRouterOptions = {
        routesFolder: [],
        dts: (0, _pathe.resolve)(nuxt.options.buildDir, declarationFile),
        ...
        async beforeWriteFiles(rootPage) {
          ...
          function addPage(parent, page) {
            const absolutePagePath = (0, _ufo.joinURL)(parent.path, page.path);
      ...
      const context = (0, _unpluginVueRouter.createRoutesContext)((0, _options2.resolveOptions)(typedRouterOptions));
      const dtsFile = (0, _pathe.resolve)(nuxt.options.buildDir, declarationFile);
      await (0, _promises.mkdir)((0, _pathe.dirname)(dtsFile), { recursive: true });
      await context.scanPages(false);
      if (nuxt.options._prepare || !nuxt.options.dev) {
        const dts = await (0, _promises.readFile)(dtsFile, "utf-8");
        (0, _kit.addTemplate)({
    ...
    const updateTemplatePaths = nuxt.options._layers.flatMap((l) => {
      ...
      return [
      (0, _pathe.resolve)(l.config.srcDir || l.cwd, dir?.pages || "pages") + "/",
      (0, _pathe.resolve)(l.config.srcDir || l.cwd, dir?.layouts || "layouts") + "/",
      (0, _pathe.resolve)(l.config.srcDir || l.cwd, dir?.middleware || "middleware") + "/"];
    ...
    nuxt.hook("app:resolve", (app) => {
      if (app.mainComponent === (0, _pathe.resolve)(nuxt.options.appDir, "components/welcome.vue")) {
        app.mainComponent = (0, _pathe.resolve)(runtimeDir, "app.vue");
      }
      app.middleware.unshift({
        name: "validate",
        path: (0, _pathe.resolve)(runtimeDir, "validate"),
        global: true
      });
    });
    nuxt.hook("app:resolve", (app) => {
      const nitro = (0, _kit.useNitro)();
      if (nitro.options.prerender.crawlLinks || Object.values(nitro.options.routeRules).some((rule) => rule.prerender)) {
        app.plugins.push({
          src: (0, _pathe.resolve)(runtimeDir, "plugins/prerender.server"),
          mode: "server"
        });
      }
    });
    ...
    function processPages(pages, currentPath = "/") {
      for (const page of pages) {
        ...
        const route = (0, _ufo.joinURL)(currentPath, page.path);
    ...
    nuxt.hook("nitro:build:before", (nitro) => {
      ...
      if (!nitro.options.static && !nitro.options.prerender.crawlLinks) {
        const routeRulesMatcher = (0, _radix.toRouteMatcher)((0, _radix.createRouter)({ routes: nitro.options.routeRules }));
        for (const route of prerenderRoutes) {
          const rules = (0, _defu.defu)({}, ...routeRulesMatcher.matchAll(route).reverse());
          if (rules.prerender) {
    ...
    nuxt.hook("imports:extend", (imports) => {
      imports.push(
        { name: "definePageMeta", as: "definePageMeta", from: (0, _pathe.resolve)(runtimeDir, "composables") },
        { name: "useLink", as: "useLink", from: "vue-router" }
      );
      if (nuxt.options.experimental.inlineRouteRules) {
        imports.push({ name: "defineRouteRules", as: "defineRouteRules", from: (0, _pathe.resolve)(runtimeDir, "composables") });
      }
    });
    if (nuxt.options.experimental.inlineRouteRules) {
      let pageToGlobMap = {};
      ...
      nuxt.hook("nitro:init", (nitro) => {
        updateRouteConfig = () => nitro.updateConfig({ routeRules: (0, _defu.defu)(inlineRules, nitro.options._config.routeRules) });
      });
      const updatePage = async function updatePage2(path) {
        const glob = pageToGlobMap[path];
        const code = path in nuxt.vfs ? nuxt.vfs[path] : await (0, _promises.readFile)(path, "utf-8");
        try {
          const extractedRule = await extractRouteRules(code, path);
          if (extractedRule) {
            if (!glob) {
              const relativePath = (0, _pathe.relative)(nuxt.options.srcDir, path);
    ...
    const componentStubPath = await (0, _kit.resolvePath)((0, _pathe.resolve)(runtimeDir, "component-stub"));
    ...
    nuxt.hook("modules:done", () => {
      (0, _kit.addBuildPlugin)(PageMetaPlugin({
        dev: nuxt.options.dev,
        sourcemap: !!nuxt.options.sourcemap.server || !!nuxt.options.sourcemap.client,
        isPage,
        routesPath: (0, _pathe.resolve)(nuxt.options.buildDir, "routes.mjs"),
        extractedKeys: nuxt.options.experimental.scanPageMeta ? extractedKeys : []
      }));
    });
    (0, _kit.addPlugin)((0, _pathe.resolve)(runtimeDir, "plugins/prefetch.client"));
    if (nuxt.options.experimental.templateRouteInjection) {
      (0, _kit.addBuildPlugin)(RouteInjectionPlugin(nuxt), { server: false });
    }
    (0, _kit.addPlugin)((0, _pathe.resolve)(runtimeDir, "plugins/router"));
    const getSources = (pages) => pages.filter((p) => Boolean(p.file)).flatMap(
      (p) => [(0, _pathe.relative)(nuxt.options.srcDir, p.file), ...(p.children?.length ? getSources(p.children) : [])]
    );
    nuxt.hook("build:manifest", (manifest) => {
      ...
      for (const [key, chunk] of Object.entries(manifest)) {
        if (chunk.src && Object.values(nuxt.apps).some((app) => app.pages?.some((page) => page.mode === "server" && page.file === (0, _pathe.join)(nuxt.options.srcDir, chunk.src)))) {
          delete manifest[key];
          continue;
        }
        if (chunk.isEntry) {
    ...
    const serverComponentRuntime = (await (0, _kit.findPath)((0, _pathe.join)(distDir, "components/runtime/server-component"))) ?? (0, _pathe.join)(distDir, "components/runtime/server-component");
    const clientComponentRuntime = (await (0, _kit.findPath)((0, _pathe.join)(distDir, "components/runtime/client-component"))) ?? (0, _pathe.join)(distDir, "components/runtime/client-component");
    (0, _kit.addTemplate)({
      filename: "routes.mjs",
      getContents({ app }) {
        if (!app.pages) {
    ...
    (0, _kit.addTemplate)({
      ...
      getContents: async ({ nuxt: nuxt2 }) => {
        ...
        const configRouterOptions = (0, _knitwork.genObjectFromRawEntries)(Object.entries(nuxt2.options.router.options).map(([key, value]) => [key, (0, _knitwork.genString)(value)]));
        return [
        ...routerOptionsFiles.map((file, index) => (0, _knitwork.genImport)(file.path, `routerOptions${index}`)),
    ...
    (0, _kit.addTypeTemplate)({
      ...
      getContents: ({ nuxt: nuxt2, app }) => {
        const composablesFile = (0, _pathe.relative)((0, _pathe.join)(nuxt2.options.buildDir, "types"), (0, _pathe.resolve)(runtimeDir, "composables"));
        const namedMiddleware = app.middleware.filter((mw) => !mw.global);
        return [
        "import type { NavigationGuard } from 'vue-router'",
        `export type MiddlewareKey = ${namedMiddleware.map((mw) => (0, _knitwork.genString)(mw.name)).join(" | ") || "never"}`,
        `declare module ${(0, _knitwork.genString)(composablesFile)} {`,
        "  interface PageMeta {",
    ...
    (0, _kit.addTypeTemplate)({
      ...
      getContents: ({ nuxt: nuxt2, app }) => {
        ...
        `export type LayoutKey = ${Object.keys(app.layouts).map((name) => (0, _knitwork.genString)(name)).join(" | ") || "string"}`,
        `declare module ${(0, _knitwork.genString)(composablesFile)} {`,
        "  interface PageMeta {",
    ...
    if (nuxt.options.experimental.viewTransition) {
      (0, _kit.addTypeTemplate)({
        ...
        getContents: ({ nuxt: nuxt2 }) => {
          const runtimeDir2 = (0, _pathe.resolve)(distDir, "pages/runtime");
          const composablesFile = (0, _pathe.relative)((0, _pathe.join)(nuxt2.options.buildDir, "types"), (0, _pathe.resolve)(runtimeDir2, "composables"));
          return [
          "import type { ComputedRef, MaybeRef } from 'vue'",
          `declare module ${(0, _knitwork.genString)(composablesFile)} {`,
          "  interface PageMeta {",
    ...
    (0, _kit.addComponent)({
      ...
      priority: 10,
      // built-in that we do not expect the user to override
      filePath: (0, _pathe.resolve)(distDir, "pages/runtime/page")
    });
  }
});
...
const UnheadImportsPlugin = (options) => (0, _unplugin.createUnplugin)(() => {
  return {
    name: "nuxt:head:unhead-imports",
    enforce: "post",
    transformInclude(id) {
      id = (0, _pathe.normalize)(id);
      return (isJS(id) || isVue(id, { type: ["script"] })) && !id.startsWith("virtual:") && !id.startsWith((0, _pathe.normalize)(distDir)) && !UNHEAD_LIB_RE.test(id);
    },
    transform(code, id) {
      ...
      const importsFromUnhead = importsToAdd.filter((specifier) => _vue.unheadVueComposablesImports[UnheadVue].includes(specifier.imported?.name));
      const importsFromHead = importsToAdd.filter((specifier) => !_vue.unheadVueComposablesImports[UnheadVue].includes(specifier.imported?.name));
      if (importsFromUnhead.length) {
        if (!(0, _pathe.normalize)(id).includes("node_modules")) {
          logger.warn(`You are importing from \`${UnheadVue}\` in \`./${(0, _pathe.relative)((0, _pathe.normalize)(options.rootDir), (0, _pathe.normalize)(id))}\`. Please import from \`#imports\` instead for full type safety.`);
        }
        s.prepend(`${(0, _knitwork.genImport)("#app/composables/head", toImports(importsFromUnhead))}
`);
      }
      if (importsFromHead.length) {
        s.prepend(`${(0, _knitwork.genImport)(UnheadVue, toImports(importsFromHead))}
`);
      }
      if (s.hasChanged()) {
        return {
...
const metaModule = (0, _kit.defineNuxtModule)({
  meta: {
    name: "nuxt:meta",
    configKey: "unhead"
  },
  setup(options, nuxt) {
    const runtimeDir = (0, _pathe.resolve)(distDir, "head/runtime");
    nuxt.options.build.transpile.push("@unhead/vue");
    const isNuxtV4 = nuxt.options._majorVersion === 4 || nuxt.options.future?.compatibilityVersion === 4;
    const componentsPath = (0, _pathe.resolve)(runtimeDir, "components");
    ...
    nuxt.options.alias["#unhead/composables"] = (0, _pathe.resolve)(runtimeDir, "composables", isNuxtV4 ? "v4" : "v3");
    (0, _kit.addBuildPlugin)(UnheadImportsPlugin({
      sourcemap: !!nuxt.options.sourcemap.server,
      rootDir: nuxt.options.rootDir
    }));
    const importPaths = nuxt.options.modulesDir.map((d) => (0, _kit.directoryToURL)(d));
    const unheadPlugins = (0, _exsolve.resolveModulePath)("@unhead/vue/plugins", { try: true, from: importPaths }) || "@unhead/vue/plugins";
    if (nuxt.options.experimental.polyfillVueUseHead) {
      nuxt.options.alias["@vueuse/head"] = (0, _exsolve.resolveModulePath)("@unhead/vue", { try: true, from: importPaths }) || "@unhead/vue";
      (0, _kit.addPlugin)({ src: (0, _pathe.resolve)(runtimeDir, "plugins/vueuse-head-polyfill") });
    }
    (0, _kit.addTemplate)({
...
export default {
  ...
  plugins: [DeprecationsPlugin, PromisesPlugin, TemplateParamsPlugin, AliasSortingPlugin],
    ...
    (0, _kit.addPlugin)({ src: (0, _pathe.resolve)(runtimeDir, "plugins/unhead") });
  }
});
...
const componentsIslandsTemplate = {
  ...
  getContents({ app, nuxt }) {
    ...
    const pageExports = pages?.filter((p) => p.mode === "server" && p.file && p.name).map((p) => {
      return `"page_${p.name}": defineAsyncComponent(${(0, _knitwork.genDynamicImport)(p.file)}.then(c => c.default || c))`;
    }) || [];
    return [
    ...
    islands.map(
      (c) => {
        ...
        return `  "${c.pascalName}": defineAsyncComponent(${(0, _knitwork.genDynamicImport)(c.filePath, { comment })}.then(c => ${exp}))`;
      }
...
const componentsTypeTemplate = {
  ...
  getContents: ({ app, nuxt }) => {
    ...
    const componentTypes = app.components.filter((c) => !c.island).map((c) => {
      const type = `typeof ${(0, _knitwork.genDynamicImport)((0, _pathe.isAbsolute)(c.filePath) ? (0, _pathe.relative)(buildDir, c.filePath).replace(NON_VUE_RE, "") : c.filePath.replace(NON_VUE_RE, ""), { wrapper: false })}['${c.export}']`;
      return [
      c.pascalName,
...
async function scanComponents(dirs, srcDir) {
  ...
  for (const dir of dirs) {
    ...
    const files = (await (0, _globby.globby)(dir.pattern, { cwd: dir.path, ignore: dir.ignore })).sort();
    if (files.length) {
      const siblings = await (0, _promises.readdir)((0, _pathe.dirname)(dir.path)).catch(() => []);
      const directory = (0, _pathe.basename)(dir.path);
      if (!siblings.includes(directory)) {
        ...
        if (caseCorrected) {
          ...
          const original = (0, _pathe.relative)(nuxt.options.srcDir, dir.path);
          const corrected = (0, _pathe.relative)(nuxt.options.srcDir, (0, _pathe.join)((0, _pathe.dirname)(dir.path), caseCorrected));
    ...
    for (const _file of files) {
      const filePath = (0, _pathe.join)(dir.path, _file);
      if (scannedPaths.find((d) => filePath.startsWith((0, _ufo.withTrailingSlash)(d))) || (0, _kit.isIgnored)(filePath)) {
        continue;
      }
      ...
      const prefixParts = [].concat(
        dir.prefix ? (0, _scule.splitByCase)(dir.prefix) : [],
        dir.pathPrefix !== false ? (0, _scule.splitByCase)((0, _pathe.relative)(dir.path, (0, _pathe.dirname)(filePath))) : []
      );
      let fileName = (0, _pathe.basename)(filePath, (0, _pathe.extname)(filePath));
      ...
      if (fileName.toLowerCase() === "index") {
        fileName = dir.pathPrefix === false ? (0, _pathe.basename)((0, _pathe.dirname)(filePath)) : "";
      }
      const suffix = mode !== "all" ? `-${mode}` : "";
      const componentNameSegments = resolveComponentNameSegments(fileName.replace(QUOTE_RE, ""), prefixParts);
      const pascalName = (0, _scule.pascalCase)(componentNameSegments);
      ...
      const kebabName = (0, _scule.kebabCase)(componentNameSegments);
      const shortPath = (0, _pathe.relative)(srcDir, filePath);
      ...
      if (!pascalName) {
        logger.warn(`Component did not resolve to a file name in \`~/${(0, _pathe.relative)(srcDir, filePath)}\`.`);
        continue;
      }
...
const LoaderPlugin = (options) => (0, _unplugin.createUnplugin)(() => {
  const exclude = options.transform?.exclude || [];
  const include = options.transform?.include || [];
  const nuxt = (0, _kit.tryUseNuxt)();
  return {
      ...
      s.replace(REPLACE_COMPONENT_TO_DIRECT_IMPORT_RE, (full, ...args) => {
        ...
        if (component) {
          ...
          if (internalInstall && nuxt?.options.test === false) {
            if (!nuxt.options.dev) {
              const relativePath = (0, _pathe.relative)(nuxt.options.rootDir, id);
              throw new Error(`[nuxt] \`~/${relativePath}\` is using \`${component.pascalName}\` which requires \`${internalInstall}\``);
            }
            Promise.resolve().then(() => jitiImport('../chunks/features.mjs').then((m) => _interopRequireWildcard(m))).then(({ installNuxtModule }) => installNuxtModule(internalInstall));
          }
          ...
          if (isServerOnly) {
            imports.add((0, _knitwork.genImport)(options.serverComponentRuntime, [{ name: "createServerComponent" }]));
          ...
          if (isClientOnly) {
            imports.add((0, _knitwork.genImport)("#app/components/client-only", [{ name: "createClientOnly" }]));
            identifier += "_client";
          }
          if (lazy) {
            const dynamicImport = `${(0, _knitwork.genDynamicImport)(component.filePath, { interopDefault: false })}.then(c => c.${component.export ?? "default"} || c)`;
            if (modifier && normalComponent) {
              const relativePath = (0, _pathe.relative)(options.srcDir, component.filePath);
              switch (modifier) {
                case "Visible":
                case "visible-":
                  imports.add((0, _knitwork.genImport)(options.clientDelayedComponentRuntime, [{ name: "createLazyVisibleComponent" }]));
                ...
                case "Interaction":
                case "interaction-":
                  imports.add((0, _knitwork.genImport)(options.clientDelayedComponentRuntime, [{ name: "createLazyInteractionComponent" }]));
                ...
                case "Idle":
                case "idle-":
                  imports.add((0, _knitwork.genImport)(options.clientDelayedComponentRuntime, [{ name: "createLazyIdleComponent" }]));
                  identifier += "_lazy_idle";
                  imports.add(`const ${identifier} = createLazyIdleComponent(${JSON.stringify(relativePath)}, ${dynamicImport})`);
                  break;
                case "MediaQuery":
                case "media-query-":
                  imports.add((0, _knitwork.genImport)(options.clientDelayedComponentRuntime, [{ name: "createLazyMediaQueryComponent" }]));
                ...
                case "If":
                case "if-":
                  imports.add((0, _knitwork.genImport)(options.clientDelayedComponentRuntime, [{ name: "createLazyIfComponent" }]));
                  identifier += "_lazy_if";
                  imports.add(`const ${identifier} = createLazyIfComponent(${JSON.stringify(relativePath)}, ${dynamicImport})`);
                  break;
                case "Never":
                case "never-":
                  imports.add((0, _knitwork.genImport)(options.clientDelayedComponentRuntime, [{ name: "createLazyNeverComponent" }]));
                  identifier += "_lazy_never";
                  imports.add(`const ${identifier} = createLazyNeverComponent(${JSON.stringify(relativePath)}, ${dynamicImport})`);
                  break;
                case "Time":
                case "time-":
                  imports.add((0, _knitwork.genImport)(options.clientDelayedComponentRuntime, [{ name: "createLazyTimeComponent" }]));
                  identifier += "_lazy_time";
                  imports.add(`const ${identifier} = createLazyTimeComponent(${JSON.stringify(relativePath)}, ${dynamicImport})`);
                  break;
              }
            } else {
              imports.add((0, _knitwork.genImport)("vue", [{ name: "defineAsyncComponent", as: "__defineAsyncComponent" }]));
              identifier += "_lazy";
              imports.add(`const ${identifier} = __defineAsyncComponent(${dynamicImport}${isClientOnly ? ".then(c => createClientOnly(c))" : ""})`);
            }
          } else {
            imports.add((0, _knitwork.genImport)(component.filePath, [{ name: component._raw ? "default" : component.export, as: identifier }]));
            if (isClientOnly) {
...
function findComponent(components, name, mode) {
  const id = (0, _scule.pascalCase)(name).replace(QUOTE_RE, "");
...
const IslandsTransformPlugin = (options) => (0, _unplugin.createUnplugin)((_options, meta) => {
  ...
  return {
    ...
    async transform(code, id) {
      ...
      const ast = (0, _ultrahtml.parse)(template[0]);
      await (0, _ultrahtml.walk)(ast, (node) => {
        if (node.type !== _ultrahtml.ELEMENT_NODE) {
          return;
        }
        if (node.name === "slot") {
...
const ComponentsChunkPlugin = (0, _unplugin.createUnplugin)((options) => {
  ...
  return {
    ...
    vite: {
      async config(config) {
        const components = options.getComponents();
        config.build = (0, _defu.default)(config.build, {
          rollupOptions: {
            input: {},
            output: {}
          }
        ...
        for (const component of components) {
          if (component.mode === "client" || component.mode === "all") {
            rollupOptions.input[component.pascalName] = await (0, _kit.resolvePath)(component.filePath);
          }
        }
      },
      async generateBundle(_opts, bundle) {
        ...
        for (const [chunkPath, chunkInfo] of Object.entries(bundle)) {
          ...
          for (const component of components) {
            if (chunkInfo.facadeModuleId && chunkInfo.exports.length > 0) {
              const { pathname } = (0, _ufo.parseURL)(decodeURIComponent((0, _nodeUrl.pathToFileURL)(chunkInfo.facadeModuleId).href));
              const isPath = (await (0, _kit.resolvePath)(component.filePath)) === pathname;
              if (isPath) {
        ...
        _nodeFs.default.writeFileSync((0, _pathe.join)(buildDir, "components-chunk.mjs"), `export const paths = ${JSON.stringify(pathAssociation, null, 2)}`);
      }
    }
  };
});
...
function TransformPlugin$1(nuxt, options) {
  const componentUnimport = (0, _unimport.createUnimport)({
    imports: [
    {
      name: "componentNames",
  ...
  return (0, _unplugin.createUnplugin)(() => ({
    name: "nuxt:components:imports",
    enforce: "post",
    transformInclude(id) {
      id = (0, _pathe.normalize)(id);
      return id.startsWith("virtual:") || id.startsWith("\0virtual:") || id.startsWith(nuxt.options.buildDir) || !(0, _kit.isIgnored)(id, void 0, nuxt);
    },
    async transform(code, id) {
      if (COMPONENT_QUERY_RE.test(id)) {
        const { search } = (0, _ufo.parseURL)(id);
        const query = (0, _vueRouter.parseQuery)(search);
        ...
        if (mode === "async") {
          ...
          return {
            code: [
            (0, _knitwork.genImport)(bare, [{ name: componentExport, as: "__component" }]),
...
const TreeShakeTemplatePlugin = (options) => (0, _unplugin.createUnplugin)(() => {
  ...
  return {
    ...
    transform(code, id) {
      ...
      if (!regexpMap.has(components)) {
        const serverPlaceholderPath = (0, _pathe.resolve)(distDir, "app/components/server-placeholder");
...
const FILENAME_RE = /([^/\\]+)\.\w+$/;
const ComponentNamePlugin = (options) => (0, _unplugin.createUnplugin)(() => {
  return {
...
const LazyHydrationTransformPlugin = (options) => (0, _unplugin.createUnplugin)(() => {
  const exclude = options.transform?.exclude || [];
  const include = options.transform?.include || [];
  return {
    ...
    async transform(code) {
      ...
      const s = new _magicString.default(code);
      try {
        const ast = (0, _ultrahtml.parse)(template);
        const components = options.getComponents();
        await (0, _ultrahtml.walk)(ast, (node) => {
          if (node.type !== 1) {
            return;
          ...
          const pascalName = (0, _scule.pascalCase)(node.name.slice(4));
          ...
          for (const attr in node.attributes) {
            ...
            const prop = (0, _scule.camelCase)(isDynamic ? attr.slice(1) : attr);
...
const isDirectory = (p) => {
  try {
    return (0, _nodeFs.statSync)(p).isDirectory();
  } catch {
    return false;
  }
};
...
const componentsModule = (0, _kit.defineNuxtModule)({
  meta: {
  ...
  async setup(componentOptions, nuxt) {
    ...
    if (nuxt.options.experimental.normalizeComponentNames) {
      (0, _kit.addBuildPlugin)(ComponentNamePlugin({ sourcemap: !!nuxt.options.sourcemap.client, getComponents }), { server: false });
      (0, _kit.addBuildPlugin)(ComponentNamePlugin({ sourcemap: !!nuxt.options.sourcemap.server, getComponents }), { client: false });
    }
    const normalizeDirs = (dir, cwd, options) => {
      ...
      if (dir === true || dir === void 0) {
        return [
        { priority: options?.priority || 0, path: (0, _pathe.resolve)(cwd, "components/islands"), island: true },
        { priority: options?.priority || 0, path: (0, _pathe.resolve)(cwd, "components/global"), global: true },
        { priority: options?.priority || 0, path: (0, _pathe.resolve)(cwd, "components") }];

      }
      if (typeof dir === "string") {
        return [
        { priority: options?.priority || 0, path: (0, _pathe.resolve)(cwd, (0, _kit.resolveAlias)(dir)) }];
      ...
      return dirs.map((_dir) => ({
        ...
        ..._dir,
        path: (0, _pathe.resolve)(cwd, (0, _kit.resolveAlias)(_dir.path))
      }));
    };
    nuxt.hook("app:resolve", async () => {
      ...
      componentDirs = allDirs.filter(isPureObjectOrString).map((dir) => {
        ...
        const dirPath = (0, _kit.resolveAlias)(dirOptions.path);
    ...
    (0, _kit.addTypeTemplate)(componentsTypeTemplate);
    (0, _kit.addPluginTemplate)(componentsPluginTemplate);
    (0, _kit.addTemplate)(componentNamesTemplate);
    (0, _kit.addTemplate)({ ...componentsIslandsTemplate, filename: "components.islands.mjs" });
    if (componentOptions.generateMetadata) {
      (0, _kit.addTemplate)(componentsMetadataTemplate);
    }
    const serverComponentRuntime = (await (0, _kit.findPath)((0, _pathe.join)(distDir, "components/runtime/server-component"))) ?? (0, _pathe.join)(distDir, "components/runtime/server-component");
    (0, _kit.addBuildPlugin)(TransformPlugin$1(nuxt, { getComponents, serverComponentRuntime, mode: "server" }), { server: true, client: false });
    (0, _kit.addBuildPlugin)(TransformPlugin$1(nuxt, { getComponents, serverComponentRuntime, mode: "client" }), { server: false, client: true });
    nuxt.hook("build:manifest", (manifest) => {
      const sourceFiles = getComponents().filter((c) => c.global).map((c) => (0, _pathe.relative)(nuxt.options.srcDir, c.filePath));
    ...
    const serverPlaceholderPath = (await (0, _kit.findPath)((0, _pathe.join)(distDir, "app/components/server-placeholder"))) ?? (0, _pathe.join)(distDir, "app/components/server-placeholder");
    nuxt.hook("app:templates", async (app) => {
      const newComponents = await scanComponents(componentDirs, nuxt.options.srcDir);
      await nuxt.callHook("components:extend", newComponents);
      for (const component of newComponents) {
        if (!component._scanned && !(component.filePath in nuxt.vfs) && (0, _pathe.isAbsolute)(component.filePath) && !(0, _nodeFs.existsSync)(component.filePath)) {
          component.filePath = (0, _exsolve.resolveModulePath)((0, _kit.resolveAlias)(component.filePath), { try: true, extensions: nuxt.options.extensions }) ?? component.filePath;
        }
    ...
    nuxt.hook("prepare:types", ({ tsConfig }) => {
      tsConfig.compilerOptions.paths["#components"] = [(0, _pathe.resolve)(nuxt.options.buildDir, "components")];
    });
    if (nuxt.options.experimental.treeshakeClientOnly) {
      (0, _kit.addBuildPlugin)(TreeShakeTemplatePlugin({ sourcemap: !!nuxt.options.sourcemap.server, getComponents }), { client: false });
    }
    const clientDelayedComponentRuntime = (await (0, _kit.findPath)((0, _pathe.join)(distDir, "components/runtime/lazy-hydrated-component"))) ?? (0, _pathe.join)(distDir, "components/runtime/lazy-hydrated-component");
    ...
    (0, _kit.addBuildPlugin)(LoaderPlugin({ ...sharedLoaderOptions, sourcemap: !!nuxt.options.sourcemap.client, mode: "client" }), { server: false });
    (0, _kit.addBuildPlugin)(LoaderPlugin({ ...sharedLoaderOptions, sourcemap: !!nuxt.options.sourcemap.server, mode: "server" }), { client: false });
    if (nuxt.options.experimental.lazyHydration) {
      (0, _kit.addBuildPlugin)(LazyHydrationTransformPlugin({
        ...sharedLoaderOptions,
        sourcemap: !!(nuxt.options.sourcemap.server || nuxt.options.sourcemap.client)
      }), { prepend: true });
    }
    if (nuxt.options.experimental.componentIslands) {
      ...
      (0, _kit.addVitePlugin)({
        ...
        handleHotUpdate(ctx) {
          ...
          const filePath = (0, _pathe.normalize)(ctx.file);
      ...
      (0, _kit.addBuildPlugin)(IslandsTransformPlugin({ getComponents, selectiveClient }), { client: false });
      nuxt.hook("vite:extendConfig", (config, { isClient }) => {
        config.plugins ||= [];
        if (isClient && selectiveClient) {
          (0, _nodeFs.writeFileSync)((0, _pathe.join)(nuxt.options.buildDir, "components-chunk.mjs"), "export const paths = {}");
          if (!nuxt.options.dev) {
            ...
            (0, _nodeFs.writeFileSync)((0, _pathe.join)(nuxt.options.buildDir, "components-chunk.mjs"), `export const paths = ${JSON.stringify(
              getComponents().filter((c) => c.mode === "client" || c.mode === "all").reduce((acc, c) => {
                ...
                const filePath = (0, _nodeFs.existsSync)(`${c.filePath}.vue`) ? `${c.filePath}.vue` : (0, _nodeFs.existsSync)(`${c.filePath}.js`) ? `${c.filePath}.js` : `${c.filePath}.ts`;
...
const TransformPlugin = ({ ctx, options, sourcemap }) => (0, _unplugin.createUnplugin)(() => {
  return {
    ...
    async transform(code, id) {
      ...
      if (imports.some((i) => i.from === "#app/composables/script-stubs") && (0, _kit.tryUseNuxt)()?.options.test === false) {
        Promise.resolve().then(() => jitiImport('../chunks/features.mjs').then((m) => _interopRequireWildcard(m))).then(({ installNuxtModule }) => installNuxtModule("@nuxt/scripts"));
      }
      if (s.hasChanged()) {
        return {
          code: s.toString(),
...
const commonPresets = [
// vue-demi (mocked)
(0, _unimport.defineUnimportPreset)({
  from: "vue-demi",
  imports: [
  "isVue2",
  "isVue3"]
...
const routerPreset = (0, _unimport.defineUnimportPreset)({
  imports: ["onBeforeRouteLeave", "onBeforeRouteUpdate"],
  from: "#app/composables/router"
});
const vuePreset = (0, _unimport.defineUnimportPreset)({
  from: "vue",
  imports: [
  // <script setup>
  "withCtx",
...
const vueTypesPreset = (0, _unimport.defineUnimportPreset)({
  from: "vue",
  type: true,
  imports: [
  "Component",
...
const importsModule = (0, _kit.defineNuxtModule)({
  meta: {
    name: "nuxt:imports",
    configKey: "imports"
  },
  defaults: (nuxt) => ({
    ...
    transform: {
      include: [
      new RegExp("^" + (0, _escapeStringRegexp.default)(nuxt.options.buildDir))],
  ...
  async setup(options, nuxt) {
    ...
    const ctx = (0, _unimport.createUnimport)({
      injectAtEnd: true,
      ...rest,
      addons: {
        addons,
    ...
    if (options.scan) {
      for (const layer of nuxt.options._layers) {
        ...
        composablesDirs.push((0, _pathe.resolve)(layer.config.srcDir, "composables"));
        composablesDirs.push((0, _pathe.resolve)(layer.config.srcDir, "utils"));
        if (isNuxtV4) {
          composablesDirs.push((0, _pathe.resolve)(layer.config.rootDir, layer.config.dir?.shared ?? "shared", "utils"));
          composablesDirs.push((0, _pathe.resolve)(layer.config.rootDir, layer.config.dir?.shared ?? "shared", "types"));
        }
        for (const dir of layer.config.imports?.dirs ?? []) {
          if (!dir) {
            continue;
          }
          composablesDirs.push((0, _pathe.resolve)(layer.config.srcDir, dir));
        }
      }
      await nuxt.callHook("imports:dirs", composablesDirs);
      composablesDirs = composablesDirs.map((dir) => (0, _pathe.normalize)(dir));
      nuxt.hook("builder:watch", (event, relativePath) => {
        if (!["addDir", "unlinkDir"].includes(event)) {
          return;
        }
    ...
    (0, _kit.addTemplate)({
      ...
      getContents: async () => (0, _unimport.toExports)(await ctx.getImports()) + '\nif (import.meta.dev) { console.warn("[nuxt] `#imports` should be transformed with real imports. There seems to be something wrong with the imports plugin.") }'
    });
    nuxt.options.alias["#imports"] = (0, _pathe.join)(nuxt.options.buildDir, "imports");
    (0, _kit.addBuildPlugin)(TransformPlugin({ ctx, options, sourcemap: !!nuxt.options.sourcemap.server || !!nuxt.options.sourcemap.client }));
    ...
    const isIgnored = (0, _kit.createIsIgnored)(nuxt);
    const regenerateImports = async () => {
      await ctx.modifyDynamicImports(async (imports) => {
        imports.length = 0;
        if (options.scan) {
          const scannedImports = await (0, _unimport.scanDirExports)(composablesDirs, {
...
function addDeclarationTemplates(ctx, options) {
  ...
  const importPaths = nuxt.options.modulesDir.map((dir) => (0, _kit.directoryToURL)(dir));
  async function cacheImportPaths(imports) {
    ...
    await Promise.all(importSource.map(async (from) => {
      ...
      let path = (0, _kit.resolveAlias)(from);
      if (!(0, _pathe.isAbsolute)(path)) {
        path = (await (0, _kit.tryResolveModule)(from, importPaths).then(async (r2) => {
          if (!r2) {
            return r2;
          }
          const { dir, name } = (0, _mlly.parseNodeModulePath)(r2);
          ...
          const subpath = await (0, _mlly.lookupNodeModuleSubpath)(r2);
          return (0, _pathe.join)(dir, name, subpath || "");
        })) ?? path;
      }
      if ((0, _nodeFs.existsSync)(path) && !(await isDirectory$1(path))) {
        path = path.replace(SUPPORTED_EXTENSION_RE, "");
      }
      if ((0, _pathe.isAbsolute)(path)) {
        path = (0, _pathe.relative)((0, _pathe.join)(nuxt.options.buildDir, "types"), path);
      }
  ...
  (0, _kit.addTypeTemplate)({
    ...
    getContents: async ({ nuxt: nuxt2 }) => (0, _unimport.toExports)(await ctx.getImports(), nuxt2.options.buildDir, true)
  });
  (0, _kit.addTypeTemplate)({
...
const createImportProtectionPatterns = (nuxt, options) => {
  ...
  for (const mod of nuxt.options.modules.filter((m) => typeof m === "string")) {
    patterns.push([
    new RegExp(`^${(0, _escapeStringRegexp.default)(mod)}$`),
  ...
  if (options.context === "nuxt-app" || options.context === "shared") {
    patterns.push([
    new RegExp((0, _escapeStringRegexp.default)((0, _pathe.relative)(nuxt.options.srcDir, (0, _pathe.resolve)(nuxt.options.srcDir, nuxt.options.serverDir || "server"))) + "\\/(api|routes|middleware|plugins)\\/"),
...
const UnctxTransformPlugin = (options) => (0, _unplugin.createUnplugin)(() => {
  const transformer = (0, _transform.createTransformer)(options.transformerOptions);
  return {
...
const TreeShakeComposablesPlugin = (options) => (0, _unplugin.createUnplugin)(() => {
...
const DevOnlyPlugin = (options) => (0, _unplugin.createUnplugin)(() => {
  return {
    ...
    transform(code) {
      ...
      for (const match of code.matchAll(DEVONLY_COMP_RE)) {
        const ast = (0, _ultrahtml.parse)(match[0]).children[0];
...
const LayerAliasingPlugin = (options) => (0, _unplugin.createUnplugin)((_options, meta) => {
  const aliases = {};
  ...
  return {
    ...
    vite: {
      resolveId: {
        ...
        async handler(id, importer) {
          ...
          const resolvedId = (0, _kit.resolveAlias)(id, aliases[layer]);
          if (resolvedId !== id) {
    ...
    transformInclude: (id) => {
      ...
      const _id = (0, _pathe.normalize)(id);
...
const addModuleTranspiles = (opts = {}) => {
  ...
  map((m) => typeof m === "string" ? m : Array.isArray(m) ? m[0] : m.src).filter((m) => typeof m === "string").map(_kit.normalizeModuleTranspilePath);
...
async function initNitro(nuxt) {
  ...
  ).filter((dir) => Boolean(dir)).map((dir) => (0, _escapeStringRegexp.default)(dir));
  const excludePattern = excludePaths.length ? [new RegExp(`node_modules\\/(?!${excludePaths.join("|")})`)] : [/node_modules/];
  const rootDirWithSlash = (0, _ufo.withTrailingSlash)(nuxt.options.rootDir);
  const modules = await (0, _kit.resolveNuxtModule)(
    rootDirWithSlash,
  ...
  if (isNuxtV4 && nuxt.options.nitro.imports !== false && nuxt.options.imports.scan !== false) {
    for (const layer of nuxt.options._layers) {
      ...
      sharedDirs.add((0, _pathe.resolve)(layer.config.rootDir, layer.config.dir?.shared ?? "shared", "utils"));
      sharedDirs.add((0, _pathe.resolve)(layer.config.rootDir, layer.config.dir?.shared ?? "shared", "types"));
    }
  }
  const mockProxy = (0, _exsolve.resolveModulePath)("mocked-exports/proxy", { from: "file:///C:/Users/admin/Documents/Front_End_Web_Hotel_Nhom_2/node_modules/nuxt/dist/shared/nuxt.iYoBCR2R.mjs" });
  const nitroConfig = (0, _defu.defu)(nuxt.options.nitro, {
    ...
    imports: {
      ...
      imports: [
      {
        as: "__buildAssetsURL",
        name: "buildAssetsURL",
        from: (0, _pathe.resolve)(distDir, "core/runtime/nitro/utils/paths")
      },
      {
      ...
      },
      {
        ...
        from: (0, _pathe.resolve)(distDir, "core/runtime/nitro/utils/config"),
        priority: -1
      }],
    ...
    analyze: !nuxt.options.test && nuxt.options.build.analyze && (nuxt.options.build.analyze === true || nuxt.options.build.analyze.enabled) ? {
      ...
      filename: (0, _pathe.join)(nuxt.options.analyzeDir, "{name}.html")
    } : false,
    scanDirs: nuxt.options._layers.map((layer) => (layer.config.serverDir || layer.config.srcDir) && (0, _pathe.resolve)(layer.cwd, layer.config.serverDir || (0, _pathe.resolve)(layer.config.srcDir, "server"))).filter(Boolean),
    renderer: (0, _pathe.resolve)(distDir, "core/runtime/nitro/handlers/renderer"),
    errorHandler: (0, _pathe.resolve)(distDir, "core/runtime/nitro/handlers/error"),
    ...
    appConfigFiles: nuxt.options._layers.map(
      (layer) => (0, _pathe.resolve)(layer.config.srcDir, "app.config")
    ),
    typescript: {
      strict: true,
      ...
      tsConfig: {
        ...
        include: [
        (0, _pathe.join)(nuxt.options.buildDir, "types/nitro-nuxt.d.ts"),
        ...modules.map((m) => (0, _pathe.join)(relativeWithDot(nuxt.options.buildDir, m), "runtime/server"))],

        exclude: [
        ...nuxt.options.modulesDir.map((m) => relativeWithDot(nuxt.options.buildDir, m)),
        // nitro generate output: https://github.com/nuxt/nuxt/blob/main/packages/nuxt/src/core/nitro.ts#L186
        relativeWithDot(nuxt.options.buildDir, (0, _pathe.resolve)(nuxt.options.rootDir, "dist"))]

      }
    },
    publicAssets: [
    nuxt.options.dev ? { dir: (0, _pathe.resolve)(nuxt.options.buildDir, "dist/client") } : {
      dir: (0, _pathe.join)(nuxt.options.buildDir, "dist/client", nuxt.options.app.buildAssetsDir),
      maxAge: 31536e3,
      baseURL: nuxt.options.app.buildAssetsDir
    },
    ...nuxt.options._layers.map((layer) => (0, _pathe.resolve)(layer.config.srcDir, (layer.config.rootDir === nuxt.options.rootDir ? nuxt.options : layer.config).dir?.public || "public")).filter((dir) => (0, _nodeFs.existsSync)(dir)).map((dir) => ({ dir }))],

    prerender: {
      ignoreUnprefixedPublicAssets: true,
      failOnError: true,
      concurrency: (0, _nodeOs.cpus)().length * 4 || 4,
    ...
    externals: {
      ...
      ...nuxt.options._layers.map((layer) => (0, _pathe.resolve)(layer.config.srcDir, "app.config"))],

      traceInclude: [
      // force include files used in generated code from the runtime-compiler
      ...(nuxt.options.vue.runtimeCompiler && !nuxt.options.experimental.externalVue ? [
      ...nuxt.options.modulesDir.reduce((targets, path) => {
        const serverRendererPath = (0, _pathe.resolve)(path, "vue/server-renderer/index.js");
        if ((0, _nodeFs.existsSync)(serverRendererPath)) {
    ...
    alias: {
      ...
      ...nuxt.options.alias,
      // Paths
      "#internal/nuxt/paths": (0, _pathe.resolve)(distDir, "core/runtime/nitro/utils/paths")
    },
    replace: {
  ...
  nitroConfig.srcDir = (0, _pathe.resolve)(nuxt.options.rootDir, nuxt.options.srcDir, nitroConfig.srcDir);
  nitroConfig.ignore ||= [];
  nitroConfig.ignore.push(
    ...(0, _kit.resolveIgnorePatterns)(nitroConfig.srcDir),
    `!${(0, _pathe.join)(nuxt.options.buildDir, "dist/client", nuxt.options.app.buildAssetsDir, "**/*")}`
  );
  nitroConfig.plugins = nitroConfig.plugins?.map((plugin) => plugin ? (0, _kit.resolveAlias)(plugin, nuxt.options.alias) : plugin);
  if (nuxt.options.experimental.appManifest) {
    const buildId = nuxt.options.runtimeConfig.app.buildId ||= nuxt.options.buildId;
    const buildTimestamp = Date.now();
    const manifestPrefix = (0, _ufo.joinURL)(nuxt.options.app.buildAssetsDir, "builds");
    const tempDir = (0, _pathe.join)(nuxt.options.buildDir, "manifest");
    nitroConfig.prerender ||= {};
    nitroConfig.prerender.ignore ||= [];
    nitroConfig.prerender.ignore.push((0, _ufo.joinURL)(nuxt.options.app.baseURL, manifestPrefix));
    nitroConfig.publicAssets.unshift(
      // build manifest
      {
        dir: (0, _pathe.join)(tempDir, "meta"),
        maxAge: 31536e3,
        baseURL: (0, _ufo.joinURL)(manifestPrefix, "meta")
      },
      // latest build
      {
        dir: tempDir,
    ...
    nuxt.options.alias["#app-manifest"] = (0, _pathe.join)(tempDir, `meta/${buildId}.json`);
    if (!nuxt.options.dev) {
      nuxt.hook("build:before", async () => {
        await _nodeFs.promises.mkdir((0, _pathe.join)(tempDir, "meta"), { recursive: true });
        await _nodeFs.promises.writeFile((0, _pathe.join)(tempDir, `meta/${buildId}.json`), JSON.stringify({}));
      });
    }
    ...
    nuxt.hook("nitro:config", (config) => {
      config.alias ||= {};
      config.alias["#app-manifest"] = (0, _pathe.join)(tempDir, `meta/${buildId}.json`);
    ...
    nuxt.hook("nitro:init", (nitro2) => {
      nitro2.hooks.hook("rollup:before", async (nitro3) => {
        ...
        const routeRulesMatcher = (0, _radix.toRouteMatcher)(
          (0, _radix.createRouter)({ routes: routeRules })
        );
        if (nitro3._prerenderedRoutes?.length) {
          ...
          for (const route of nitro3._prerenderedRoutes) {
            if (!route.error && route.route.endsWith(payloadSuffix)) {
              ...
              const rules = (0, _defu.defu)({}, ...routeRulesMatcher.matchAll(url).reverse());
              if (!rules.prerender) {
        ...
        const manifest = {
          ...
          matcher: (0, _radix.exportMatcher)(routeRulesMatcher),
          prerendered: nuxt.options.dev ? [] : [...prerenderedRoutes]
        };
        await _nodeFs.promises.mkdir((0, _pathe.join)(tempDir, "meta"), { recursive: true });
        await _nodeFs.promises.writeFile((0, _pathe.join)(tempDir, "latest.json"), JSON.stringify({
          id: buildId,
          timestamp: buildTimestamp
        }));
        await _nodeFs.promises.writeFile((0, _pathe.join)(tempDir, `meta/${buildId}.json`), JSON.stringify(manifest));
      });
    });
  }
  if (!nuxt.options.experimental.appManifest) {
  ...
  if (nuxt.options.experimental.respectNoSSRHeader) {
    ...
    nitroConfig.handlers.push({
      handler: (0, _pathe.resolve)(distDir, "core/runtime/nitro/middleware/no-ssr"),
      middleware: true
    });
  }
  nitroConfig.rollupConfig.plugins = (await nitroConfig.rollupConfig.plugins) || [];
  nitroConfig.rollupConfig.plugins = toArray(nitroConfig.rollupConfig.plugins);
  const sharedDir = (0, _ufo.withTrailingSlash)((0, _pathe.resolve)(nuxt.options.rootDir, nuxt.options.dir.shared));
  const relativeSharedDir = (0, _ufo.withTrailingSlash)((0, _pathe.relative)(nuxt.options.rootDir, (0, _pathe.resolve)(nuxt.options.rootDir, nuxt.options.dir.shared)));
  const sharedPatterns = [/^#shared\//, new RegExp("^" + (0, _escapeStringRegexp.default)(sharedDir)), new RegExp("^" + (0, _escapeStringRegexp.default)(relativeSharedDir))];
  nitroConfig.rollupConfig.plugins.push(
    _impound.ImpoundPlugin.rollup({
  ...
  const basePath = nitroConfig.typescript.tsConfig.compilerOptions?.baseUrl ? (0, _pathe.resolve)(nuxt.options.buildDir, nitroConfig.typescript.tsConfig.compilerOptions?.baseUrl) : nuxt.options.buildDir;
  ...
  for (const _alias in aliases) {
    ...
    const absolutePath = (0, _pathe.resolve)(basePath, aliases[alias]);
    const stats = await _nodeFs.promises.stat(absolutePath).catch(
      () => null
      /* file does not exist */
    );
  ...
  const nitro = await (0, _nitropack.createNitro)(nitroConfig, {
  ...
  const cacheDir = (0, _pathe.resolve)(nuxt.options.buildDir, "cache/nitro/prerender");
  const cacheDriverPath = (0, _pathe.join)(distDir, "core/runtime/nitro/utils/cache-driver.js");
  await _nodeFs.promises.rm(cacheDir, { recursive: true, force: true }).catch(() => {
  });
  nitro.options._config.storage = (0, _defu.defu)(nitro.options._config.storage, {
    "internal:nuxt:prerender": {
      // TODO: resolve upstream where file URLs are not being resolved/inlined correctly
      driver: _stdEnv.isWindows ? (0, _nodeUrl.pathToFileURL)(cacheDriverPath).href : cacheDriverPath,
      base: cacheDir
    }
  });
  nuxt._nitro = nitro;
  ...
  const devMiddlewareHandler = (0, _h.dynamicEventHandler)();
  nitro.options.devHandlers.unshift({ handler: devMiddlewareHandler });
  nitro.options.devHandlers.push(...nuxt.options.devServerHandlers);
  nitro.options.handlers.unshift({
    route: "/__nuxt_error",
    lazy: true,
    handler: (0, _pathe.resolve)(distDir, "core/runtime/nitro/handlers/renderer")
  });
  ...
  nuxt.hook("prepare:types", async (opts) => {
    if (!nuxt.options.dev) {
      await (0, _nitropack.scanHandlers)(nitro);
      await (0, _nitropack.writeTypes)(nitro);
    }
    opts.tsConfig.exclude ||= [];
    opts.tsConfig.exclude.push((0, _pathe.relative)(nuxt.options.buildDir, (0, _pathe.resolve)(nuxt.options.rootDir, nitro.options.output.dir)));
    opts.references.push({ path: (0, _pathe.resolve)(nuxt.options.buildDir, "types/nitro.d.ts") });
  });
  ...
  if (!nuxt.options.dev) {
    nitro.hooks.hook("rollup:before", async (nitro2) => {
      await (0, _nitropack.copyPublicAssets)(nitro2);
      await nuxt.callHook("nitro:build:public-assets", nitro2);
    });
  }
  async function symlinkDist() {
    if (nitro.options.static) {
      const distDir2 = (0, _pathe.resolve)(nuxt.options.rootDir, "dist");
      if (!(0, _nodeFs.existsSync)(distDir2)) {
        await _nodeFs.promises.symlink(nitro.options.output.publicDir, distDir2, "junction").catch(() => {
        });
      }
    }
  }
  nuxt.hook("build:done", async () => {
    ...
    await (0, _nitropack.prepare)(nitro);
    if (nuxt.options.dev) {
      return (0, _nitropack.build)(nitro);
    }
    await (0, _nitropack.prerender)(nitro);
    _kit.logger.restoreAll();
    await (0, _nitropack.build)(nitro);
    _kit.logger.wrapAll();
    await symlinkDist();
  });
  if (nuxt.options.dev) {
    for (const builder of ["webpack", "rspack"]) {
      nuxt.hook(`${builder}:compile`, ({ name, compiler }) => {
        if (name === "server") {
          ...
          nitro.options.virtual["#build/dist/server/server.mjs"] = () => memfs.readFileSync((0, _pathe.join)(nuxt.options.buildDir, "dist/server/server.mjs"), "utf-8");
        }
      });
    ...
    nuxt.server = (0, _nitropack.createDevServer)(nitro);
...
function relativeWithDot(from, to) {
  return (0, _pathe.relative)(from, to).replace(RELATIVE_RE, "./$1") || ".";
}
async function spaLoadingTemplatePath(nuxt) {
  if (typeof nuxt.options.spaLoadingTemplate === "string") {
    return (0, _pathe.resolve)(nuxt.options.srcDir, nuxt.options.spaLoadingTemplate);
  }
  const possiblePaths = nuxt.options._layers.map((layer) => (0, _pathe.resolve)(layer.config.srcDir, layer.config.dir?.app || "app", "spa-loading-template.html"));
  return (await (0, _kit.findPath)(possiblePaths)) ?? (0, _pathe.resolve)(nuxt.options.srcDir, nuxt.options.dir?.app || "app", "spa-loading-template.html");
}
async function spaLoadingTemplate(nuxt) {
  ...
  try {
    if ((0, _nodeFs.existsSync)(spaLoadingTemplate2)) {
      return (0, _nodeFs.readFileSync)(spaLoadingTemplate2, "utf-8").trim();
    }
  } catch {
  }
  ...
  if (nuxt.options.spaLoadingTemplate) {
    _kit.logger.warn(`Could not load custom \`spaLoadingTemplate\` path as it does not exist: \`${nuxt.options.spaLoadingTemplate}\`.`);
  }
  return "";
}

const schemaModule = (0, _kit.defineNuxtModule)({
  meta: {
  ...
  async setup(_, nuxt) {
    ...
    const resolver = (0, _kit.createResolver)("file:///C:/Users/admin/Documents/Front_End_Web_Hotel_Nhom_2/node_modules/nuxt/dist/shared/nuxt.iYoBCR2R.mjs");
    const _resolveSchema = (0, _jiti.createJiti)((0, _nodeUrl.fileURLToPath)("file:///C:/Users/admin/Documents/Front_End_Web_Hotel_Nhom_2/node_modules/nuxt/dist/shared/nuxt.iYoBCR2R.mjs"), {
      cache: false,
      transformOptions: {
        babel: {
    ...
    if (nuxt.options.dev) {
      const onChange = (0, _perfectDebounce.debounce)(async () => {
        schema = await resolveSchema$1();
        await writeSchema(schema);
      });
      if (nuxt.options.experimental.watcher === "parcel") {
        try {
          const { subscribe } = await (0, _kit.importModule)("@parcel/watcher", {
            url: [nuxt.options.rootDir, ...nuxt.options.modulesDir].map((dir) => (0, _kit.directoryToURL)(dir))
          });
          for (const layer of nuxt.options._layers) {
      ...
      const watcher = (0, _chokidar.watch)(dirsToWatch, {
    ...
    async function resolveSchema$1() {
      ...
      for (const layer of nuxt.options._layers) {
        const filePath = await resolver.resolvePath((0, _pathe.resolve)(layer.config.rootDir, "nuxt.schema"));
        if (filePath && (0, _nodeFs.existsSync)(filePath)) {
          let loadedConfig;
          try {
      ...
      const schemas = await Promise.all(
        schemaDefs.map((schemaDef) => (0, _untyped.resolveSchema)(schemaDef))
      );
      const schema2 = (0, _defu.defu)(...schemas);
    ...
    async function writeSchema(schema2) {
      ...
      await (0, _promises.mkdir)((0, _pathe.resolve)(nuxt.options.buildDir, "schema"), { recursive: true });
      await (0, _promises.writeFile)(
        (0, _pathe.resolve)(nuxt.options.buildDir, "schema/nuxt.schema.json"),
        JSON.stringify(schema2, null, 2),
        "utf8"
      );
      const _types = (0, _untyped.generateTypes)(schema2, {
        addExport: true,
...
`;
      const typesPath = (0, _pathe.resolve)(
        nuxt.options.buildDir,
        "schema/nuxt.schema.d.ts"
      );
      await (0, _promises.writeFile)(typesPath, types, "utf8");
...
function extractMetadata(code, loader = "ts") {
  ...
  parseAndWalk(code, `file.${loader}`, (node) => {
    ...
    if (plugin?.type === "ObjectExpression") {
      meta = (0, _defu.defu)(extractMetaFromObject(plugin.properties), meta);
    }
...
const RemovePluginMetadataPlugin = (nuxt) => (0, _unplugin.createUnplugin)(() => {
  return {
    ...
    transform(code, id) {
      ...
      const exports = (0, _mlly.findExports)(code);
...
const AsyncContextInjectionPlugin = (nuxt) => (0, _unplugin.createUnplugin)(() => {
  return {
...
const ComposableKeysPlugin = (options) => (0, _unplugin.createUnplugin)(() => {
  ...
  const KEYED_FUNCTIONS_RE = new RegExp(`\\b(${[...keyedFunctions].map((f) => (0, _escapeStringRegexp.default)(f)).join("|")})\\b`);
  return {
    name: "nuxt:composable-keys",
    enforce: "post",
    transformInclude(id) {
      const { pathname, search } = (0, _ufo.parseURL)(decodeURIComponent((0, _nodeUrl.pathToFileURL)(id).href));
      return !NUXT_LIB_RE.test(pathname) && SUPPORTED_EXT_RE.test(pathname) && (0, _ufo.parseQuery)(search).type !== "style" && !(0, _ufo.parseQuery)(search).macro;
    },
    transform(code, id) {
      ...
      let imports;
      let count = 0;
      const relativeID = (0, _pathe.isAbsolute)(id) ? (0, _pathe.relative)(options.rootDir, id) : id;
      const { pathname: relativePathname } = (0, _ufo.parseURL)(relativeID);
      const scopeTracker = new ScopeTracker({
        keepExitedScopes: true
      });
      ...
      walk(ast, {
        ...
        enter(node) {
          ...
          s.appendLeft(
            ...
            (node.arguments.length && !endsWithComma ? ", " : "") + "'$" + (0, _ohash.hash)(`${relativeID}-${++count}`).slice(0, 10) + "'"
          );
        }
      });
      if (s.hasChanged()) {
...
function detectImportNames(code, composableMeta) {
  ...
  for (const i of (0, _mlly.findStaticImports)(code)) {
    if (NUXT_IMPORT_RE.test(i.specifier)) {
      continue;
    }
    const { namedImports = {}, defaultImport, namespacedImport } = (0, _mlly.parseStaticImport)(i);
...
function ResolveDeepImportsPlugin(nuxt) {
  ...
  return {
    ...
    async resolveId(id, importer) {
      if (!importer || (0, _pathe.isAbsolute)(id) || !(0, _pathe.isAbsolute)(importer) && !VIRTUAL_RE.test(importer) || exclude.some((e) => id.startsWith(e))) {
        return;
      }
      const normalisedId = (0, _kit.resolveAlias)((0, _pathe.normalize)(id), nuxt.options.alias);
      ...
      if (nuxt.options.experimental.templateImportResolution !== false && isNuxtTemplate) {
        const template = nuxt.options.build.templates.find((t) => (0, _pathe.resolve)(nuxt.options.buildDir, t.filename) === normalisedImporter);
        if (template?._path) {
      ...
      const dir = (0, _mlly.parseNodeModulePath)(normalisedImporter).dir || pkgDir;
      const res = await this.resolve?.(normalisedId, dir, { skipSelf: true });
      if (res !== void 0 && res !== null) {
        return res;
      }
      const path = (0, _exsolve.resolveModulePath)(id, {
        from: [dir, ...nuxt.options.modulesDir].map((d) => (0, _kit.directoryToURL)(d)),
        suffixes: ["", "index"],
        conditions,
        try: true
      });
      ...
      return (0, _pathe.normalize)(path);
    }
  };
}
...
function ResolveExternalsPlugin(nuxt) {
  ...
  return {
    ...
    async configResolved() {
      if (!nuxt.options.dev) {
        const runtimeNitroDependencies = (await (0, _kit.tryImportModule)("nitropack/package.json", {
          url: new URL("file:///C:/Users/admin/Documents/Front_End_Web_Hotel_Nhom_2/node_modules/nuxt/dist/shared/nuxt.iYoBCR2R.mjs")
    ...
    async resolveId(id, importer) {
      ...
      if (res !== void 0 && res !== null) {
        if (res.id === id) {
          res.id = (0, _exsolve.resolveModulePath)(res.id, {
            try: true,
            from: importer,
      ...
      parseAndWalk(code, id, (node) => {
        ...
        if (node.callee.name === "onPrehydrate") {
          ...
          promises.push(p.then(({ code: result }) => {
            ...
            if (needsAttr) {
              args.push(JSON.stringify((0, _ohash.hash)(result).slice(0, 10)));
            }
...
const VirtualFSPlugin = (nuxt, options) => (0, _unplugin.createUnplugin)(() => {
  ...
  return {
    ...
    resolveId(id, importer) {
      id = (0, _kit.resolveAlias)(id, alias);
      if (process.platform === "win32" && (0, _pathe.isAbsolute)(id)) {
        id = (0, _pathe.resolve)(id);
      }
      ...
      if (importer && RELATIVE_ID_RE.test(id)) {
        const path = (0, _pathe.resolve)((0, _pathe.dirname)(withoutPrefix(decodeURIComponent(importer))), id);
...
function createNuxt(options) {
  ...
  hooks.callHook = (...args) => (0, _kit.runWithNuxtContext)(nuxt, () => callHook(...args));
  hooks.callHookParallel = (...args) => (0, _kit.runWithNuxtContext)(nuxt, () => callHookParallel(...args));
  hooks.callHookWith = (...args) => (0, _kit.runWithNuxtContext)(nuxt, () => callHookWith(...args));
  const nuxt = {
    __name: (0, _nodeCrypto.randomUUID)(),
    _version: version,
    _asyncLocalStorageModule: options.experimental.debugModuleMutation ? new _nodeAsync_hooks.AsyncLocalStorage() : void 0,
    hooks,
    callHook: hooks.callHook,
    addHooks: hooks.addHooks,
    hook: hooks.hook,
    ready: () => (0, _kit.runWithNuxtContext)(nuxt, () => initNuxt(nuxt)),
    close: () => hooks.callHook("close", nuxt),
    vfs: {},
    apps: {},
    runWithContext: (fn) => (0, _kit.runWithNuxtContext)(nuxt, fn),
    options
  };
  if (options.experimental.debugModuleMutation) {
    ...
    Object.defineProperty(nuxt, "options", {
      get() {
        ...
        const proxied = (0, _onChange.default)(options, (keys, newValue, previousValue, applyData) => {
  ...
  if (!_kit.nuxtCtx.tryUse()) {
    _kit.nuxtCtx.set(nuxt);
    nuxt.hook("close", () => {
      _kit.nuxtCtx.unset();
    });
  }
...
async function initNuxt(nuxt) {
  ...
  nuxt.options.compatibilityDate = (0, _compatx.resolveCompatibilityDatesFromEnv)(nuxt.options.compatibilityDate);
  if (!nuxt.options.compatibilityDate.default) {
    const todaysDate = (0, _compatx.formatDate)(/* @__PURE__ */new Date());
    nuxt.options.compatibilityDate.default = fallbackCompatibilityDate;
    const shouldShowPrompt = nuxt.options.dev && _stdEnv.hasTTY && !_stdEnv.isCI;
    if (!shouldShowPrompt) {
      logger.info(`Using \`${fallbackCompatibilityDate}\` as fallback compatibility date.`);
    }
    async function promptAndUpdate() {
      const result = await _consola.default.prompt(`Do you want to update your ${(0, _utils.colorize)("cyan", "nuxt.config")} to set ${(0, _utils.colorize)("cyan", `compatibilityDate: '${todaysDate}'`)}?`, {
        type: "confirm",
        default: true
      });
      if (result !== true) {
      ...
      try {
        const res = await (0, _update.updateConfig)({
          configFile: "nuxt.config",
          cwd: nuxt.options.rootDir,
          async onCreate({ configFile }) {
            const shallCreate = await _consola.default.prompt(`Do you want to create ${(0, _utils.colorize)("cyan", (0, _pathe.relative)(nuxt.options.rootDir, configFile))}?`, {
              type: "confirm",
              default: true
            });
            if (shallCreate !== true) {
        ...
        if (res?.configFile) {
          nuxt.options.compatibilityDate = (0, _compatx.resolveCompatibilityDatesFromEnv)(todaysDate);
          _consola.default.success(`Compatibility date set to \`${todaysDate}\` in \`${(0, _pathe.relative)(nuxt.options.rootDir, res.configFile)}\``);
          return;
        }
      } catch (err) {
        const message = err instanceof Error ? err.message : err;
        _consola.default.error(`Failed to update config: ${message}`);
      }
      logger.info(`Using \`${fallbackCompatibilityDate}\` as fallback compatibility date.`);
    }
  ...
  const layersDir = (0, _ufo.withTrailingSlash)((0, _pathe.resolve)(nuxt.options.rootDir, "layers"));
  nuxt.hook("builder:watch", (event, relativePath) => {
    const path = (0, _pathe.resolve)(nuxt.options.srcDir, relativePath);
  ...
  if (nuxt.options.typescript.builder !== false) {
    ...
    if (environmentTypes) {
      ...
      (0, _kit.addTypeTemplate)({
        ...
        getContents: () => (0, _knitwork.genImport)(environmentTypes)
      });
    }
  }
  const packageJSON = await (0, _pkgTypes.readPackageJSON)(nuxt.options.rootDir).catch(() => ({}));
  const NESTED_PKG_RE = /^[^@]+\//;
  ...
  nuxt.options.nitro.typescript = (0, _defu.default)(nuxt.options.nitro.typescript, {
    tsConfig: { compilerOptions: { paths: { ...paths } } }
  });
  nuxt.hook("prepare:types", (opts) => {
    opts.references.push({ types: "nuxt" });
    opts.references.push({ path: (0, _pathe.resolve)(nuxt.options.buildDir, "types/app-defaults.d.ts") });
    opts.references.push({ path: (0, _pathe.resolve)(nuxt.options.buildDir, "types/plugins.d.ts") });
    if (nuxt.options.typescript.shim) {
      opts.references.push({ path: (0, _pathe.resolve)(nuxt.options.buildDir, "types/vue-shim.d.ts") });
    }
    opts.references.push({ path: (0, _pathe.resolve)(nuxt.options.buildDir, "types/build.d.ts") });
    opts.references.push({ path: (0, _pathe.resolve)(nuxt.options.buildDir, "types/schema.d.ts") });
    opts.references.push({ path: (0, _pathe.resolve)(nuxt.options.buildDir, "types/app.config.d.ts") });
    opts.tsConfig.compilerOptions = (0, _defu.default)(opts.tsConfig.compilerOptions, { paths: { ...paths } });
    for (const layer of nuxt.options._layers) {
      const declaration = (0, _pathe.join)(layer.cwd, "index.d.ts");
      if ((0, _nodeFs.existsSync)(declaration)) {
  ...
  if (nuxt.options.scripts) {
    if (!nuxt.options._modules.some((m) => m === "@nuxt/scripts" || m === "@nuxt/scripts-nightly")) {
      await Promise.resolve().then(() => jitiImport('../chunks/features.mjs').then((m) => _interopRequireWildcard(m))).then(({ installNuxtModule }) => installNuxtModule("@nuxt/scripts"));
    }
  }
  await initParser();
  (0, _kit.addBuildPlugin)(VirtualFSPlugin(nuxt, { mode: "server" }), { client: false });
  (0, _kit.addBuildPlugin)(VirtualFSPlugin(nuxt, { mode: "client", alias: { "#internal/nitro": (0, _pathe.join)(nuxt.options.buildDir, "nitro.client.mjs") } }), { server: false });
  (0, _kit.addBuildPlugin)(RemovePluginMetadataPlugin(nuxt));
  (0, _kit.addBuildPlugin)(ComposableKeysPlugin({
    sourcemap: !!nuxt.options.sourcemap.server || !!nuxt.options.sourcemap.client,
    rootDir: nuxt.options.rootDir,
  ...
  (0, _kit.addVitePlugin)(() => _impound.ImpoundPlugin.vite(sharedProtectionConfig), { server: false });
  (0, _kit.addWebpackPlugin)(() => _impound.ImpoundPlugin.webpack(sharedProtectionConfig), { server: false });
  const nuxtProtectionConfig = {
    cwd: nuxt.options.rootDir,
    // Exclude top-level resolutions by plugins
    exclude: [(0, _pathe.relative)(nuxt.options.rootDir, (0, _pathe.join)(nuxt.options.srcDir, "index.html")), ...sharedPatterns],
    patterns: createImportProtectionPatterns(nuxt, { context: "nuxt-app" })
  };
  (0, _kit.addVitePlugin)(() => Object.assign(_impound.ImpoundPlugin.vite({ ...nuxtProtectionConfig, error: false }), { name: "nuxt:import-protection" }), { client: false });
  (0, _kit.addVitePlugin)(() => Object.assign(_impound.ImpoundPlugin.vite({ ...nuxtProtectionConfig, error: true }), { name: "nuxt:import-protection" }), { server: false });
  (0, _kit.addWebpackPlugin)(() => _impound.ImpoundPlugin.webpack(nuxtProtectionConfig));
  (0, _kit.addVitePlugin)(() => ResolveDeepImportsPlugin(nuxt), { client: false });
  (0, _kit.addVitePlugin)(() => ResolveDeepImportsPlugin(nuxt), { server: false });
  (0, _kit.addVitePlugin)(() => ResolveExternalsPlugin(nuxt), { client: false, prepend: true });
  (0, _kit.addBuildPlugin)(PrehydrateTransformPlugin({ sourcemap: !!nuxt.options.sourcemap.server || !!nuxt.options.sourcemap.client }));
  if (nuxt.options.experimental.localLayerAliases) {
    (0, _kit.addBuildPlugin)(LayerAliasingPlugin({
      sourcemap: !!nuxt.options.sourcemap.server || !!nuxt.options.sourcemap.client,
      dev: nuxt.options.dev,
  ...
  nuxt.hook("modules:done", () => {
    (0, _kit.addBuildPlugin)(UnctxTransformPlugin({
      sourcemap: !!nuxt.options.sourcemap.server || !!nuxt.options.sourcemap.client,
      transformerOptions: {
    ...
    if (Object.keys(nuxt.options.optimization.treeShake.composables.server).length) {
      (0, _kit.addBuildPlugin)(TreeShakeComposablesPlugin({
        sourcemap: !!nuxt.options.sourcemap.server,
  ...
  if (!nuxt.options.dev) {
    (0, _kit.addBuildPlugin)(DevOnlyPlugin({
      sourcemap: !!nuxt.options.sourcemap.server || !!nuxt.options.sourcemap.client
    }));
  }
  if (nuxt.options.dev) {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/check-if-layout-used"));
  }
  if (nuxt.options.dev && nuxt.options.features.devLogs) {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/dev-server-logs"));
    (0, _kit.addServerPlugin)((0, _pathe.resolve)(distDir, "core/runtime/nitro/plugins/dev-server-logs"));
    nuxt.options.nitro = (0, _defu.default)(nuxt.options.nitro, {
      externals: {
  ...
  if (nuxt.options.experimental.asyncContext) {
    (0, _kit.addBuildPlugin)(AsyncContextInjectionPlugin(nuxt), { client: false });
  }
  if (nuxt.options.features.noScripts && !nuxt.options.dev) {
    nuxt.hook("build:manifest", async (manifest) => {
      for (const chunk of Object.values(manifest)) {
        if (chunk.resourceType === "script") {
          await (0, _promises.rm)((0, _pathe.resolve)(nuxt.options.buildDir, "dist/client", (0, _ufo.withoutLeadingSlash)(nuxt.options.app.buildAssetsDir), chunk.file), { force: true });
          chunk.file = "";
        }
      }
    });
  ...
  const locallyScannedLayersDirs = nuxt.options._layers.map((l) => (0, _pathe.resolve)(l.cwd, "layers").replace(/\/?$/, "/"));
  nuxt.options.modulesDir.push(...nuxt.options._layers.filter((l) => l.cwd !== nuxt.options.rootDir && locallyScannedLayersDirs.every((dir) => !l.cwd.startsWith(dir))).map((l) => (0, _pathe.resolve)(l.cwd, "node_modules")));
  ...
  for (const _mod of nuxt.options.modules) {
    ...
    const modAlias = (0, _kit.resolveAlias)(mod);
    const modPath = (0, _exsolve.resolveModulePath)(modAlias, {
      try: true,
      from: nuxt.options.modulesDir.map((m) => (0, _kit.directoryToURL)(m.replace(/\/node_modules\/?$/, "/"))),
  ...
  for (const config of nuxt.options._layers.map((layer) => layer.config).reverse()) {
    ...
    const layerModules = await (0, _kit.resolveFiles)(config.srcDir, [
  ...
  if (nuxt.options.dev || !(typeof islandsConfig === "object" && islandsConfig.selectiveClient === "deep")) {
    (0, _kit.addComponent)({
      ...
      priority: 10,
      // built-in that we do not expect the user to override
      filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/welcome")
    });
  }
  (0, _kit.addComponent)({
    name: "NuxtLayout",
    priority: 10,
    // built-in that we do not expect the user to override
    filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/nuxt-layout")
  });
  (0, _kit.addComponent)({
    name: "NuxtErrorBoundary",
    priority: 10,
    // built-in that we do not expect the user to override
    filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/nuxt-error-boundary")
  });
  (0, _kit.addComponent)({
    name: "ClientOnly",
    priority: 10,
    // built-in that we do not expect the user to override
    filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/client-only")
  });
  (0, _kit.addComponent)({
    name: "DevOnly",
    priority: 10,
    // built-in that we do not expect the user to override
    filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/dev-only")
  });
  (0, _kit.addComponent)({
    name: "ServerPlaceholder",
    priority: 10,
    // built-in that we do not expect the user to override
    filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/server-placeholder")
  });
  (0, _kit.addComponent)({
    name: "NuxtLink",
    priority: 10,
    // built-in that we do not expect the user to override
    filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/nuxt-link")
  });
  (0, _kit.addComponent)({
    name: "NuxtLoadingIndicator",
    priority: 10,
    // built-in that we do not expect the user to override
    filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/nuxt-loading-indicator")
  });
  (0, _kit.addComponent)({
    name: "NuxtRouteAnnouncer",
    priority: 10,
    // built-in that we do not expect the user to override
    filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/nuxt-route-announcer"),
    mode: "client"
  });
  if (nuxt.options.experimental.clientFallback) {
    (0, _kit.addComponent)({
      ...
      priority: 10,
      // built-in that we do not expect the user to override
      filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/client-fallback.client"),
      mode: "client"
    });
    (0, _kit.addComponent)({
      name: "NuxtClientFallback",
      ...
      priority: 10,
      // built-in that we do not expect the user to override
      filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/client-fallback.server"),
      mode: "server"
    });
  }
  for (const name of ["NuxtImg", "NuxtPicture"]) {
    (0, _kit.addComponent)({
      ...
      export: name,
      priority: -1,
      filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/nuxt-stubs"),
      // @ts-expect-error TODO: refactor to nuxi
      _internal_install: "@nuxt/image"
    });
  }
  if (nuxt.options.builder === "@nuxt/webpack-builder") {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/preload.server"));
  }
  if (nuxt.options.debug && nuxt.options.debug.hooks && (nuxt.options.debug.hooks === true || nuxt.options.debug.hooks.client)) {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/debug-hooks"));
  }
  if (nuxt.options.experimental.browserDevtoolsTiming) {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/browser-devtools-timing.client"));
  }
  for (const [key, options] of modulesToInstall) {
    await (0, _kit.installModule)(key, options);
  }
  nuxt._ignore = (0, _ignore.default)(nuxt.options.ignoreOptions);
  nuxt._ignore.add((0, _kit.resolveIgnorePatterns)());
  ...
  if (nuxt.options.experimental.componentIslands) {
    (0, _kit.addComponent)({
      ...
      priority: 10,
      // built-in that we do not expect the user to override
      filePath: (0, _pathe.resolve)(nuxt.options.appDir, "components/nuxt-island")
    });
    if (!nuxt.options.ssr && nuxt.options.experimental.componentIslands !== "auto") {
      nuxt.options.ssr = true;
      nuxt.options.nitro.routeRules ||= {};
      nuxt.options.nitro.routeRules["/**"] = (0, _defu.default)(nuxt.options.nitro.routeRules["/**"], { ssr: false });
    }
  }
  if (!nuxt.options.dev && nuxt.options.experimental.payloadExtraction) {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/payload.client"));
  }
  if (nuxt.options.experimental.crossOriginPrefetch) {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/cross-origin-prefetch.client"));
  }
  if (nuxt.options.experimental.emitRouteChunkError === "automatic") {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/chunk-reload.client"));
  }
  if (nuxt.options.experimental.emitRouteChunkError === "automatic-immediate") {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/chunk-reload-immediate.client"));
  }
  if (nuxt.options.experimental.restoreState) {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/restore-state.client"));
  }
  if (nuxt.options.experimental.viewTransition) {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/view-transitions.client"));
  }
  if (nuxt.options.experimental.renderJsonPayloads) {
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/revive-payload.client"));
    (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/revive-payload.server"));
  }
  if (nuxt.options.experimental.appManifest) {
    (0, _kit.addRouteMiddleware)({
      name: "manifest-route-rule",
      path: (0, _pathe.resolve)(nuxt.options.appDir, "middleware/manifest-route-rule"),
      global: true
    });
    if (nuxt.options.experimental.checkOutdatedBuildInterval !== false) {
      (0, _kit.addPlugin)((0, _pathe.resolve)(nuxt.options.appDir, "plugins/check-outdated-build.client"));
    }
  }
  if (nuxt.options.experimental.navigationRepaint) {
    (0, _kit.addPlugin)({
      src: (0, _pathe.resolve)(nuxt.options.appDir, "plugins/navigation-repaint.client")
    });
  }
...
export default defineNuxtPlugin({
  ...
  nuxt.hooks.hook("builder:watch", (event, relativePath) => {
    ...
    const layerRelativePaths = nuxt.options._layers.map((l) => (0, _pathe.relative)(l.config.srcDir || l.cwd, path));
    ...
    if (event === "addDir" && path === (0, _pathe.resolve)(nuxt.options.srcDir, "app")) {
  ...
  nuxt.options.build.transpile = nuxt.options.build.transpile.map((t) => typeof t === "string" ? (0, _pathe.normalize)(t) : t);
  addModuleTranspiles();
  ...
  if (!(0, _semver.satisfies)(nuxt._version, nuxt.options.future.compatibilityVersion + ".x")) {
    logger.info(`Running with compatibility version \`${nuxt.options.future.compatibilityVersion}\``);
  }
  await nuxt.callHook("ready", nuxt);
}
async function loadNuxt(opts) {
  const options = await (0, _kit.loadNuxtConfig)(opts);
  options.appDir = options.alias["#app"] = (0, _pathe.resolve)(distDir, "app");
  ...
  if (!options._modules.some((m) => m === "@nuxt/scripts" || m === "@nuxt/scripts-nightly")) {
    options.imports = (0, _defu.default)(options.imports, {
      presets: [scriptsStubsPreset]
    });
  }
  if (options.builder === "@nuxt/webpack-builder") {
    if (!(await Promise.resolve().then(() => jitiImport('../chunks/features.mjs').then((m) => _interopRequireWildcard(m))).then((r) => r.ensurePackageInstalled("@nuxt/webpack-builder", {
  ...
  options._modules.push([importsModule, {
    transform: {
      include: options._layers.filter((i) => i.cwd && i.cwd.includes("node_modules")).map((i) => new RegExp(`(^|\\/)${(0, _escapeStringRegexp.default)(i.cwd.split("node_modules/").pop())}(\\/|$)(?!node_modules\\/)`))
    }
  }]);
  options._modules.push(schemaModule);
  options.modulesDir.push((0, _pathe.resolve)(options.workspaceDir, "node_modules"));
  options.modulesDir.push((0, _pathe.resolve)(pkgDir, "node_modules"));
  options.build.transpile.push(
    "mocked-exports",
    "std-env"
    // we need to statically replace process.env when used in runtime code
  );
  options.alias["vue-demi"] = (0, _pathe.resolve)(options.appDir, "compat/vue-demi");
  options.alias["@vue/composition-api"] = (0, _pathe.resolve)(options.appDir, "compat/capi");
  ...
  nuxt.runWithContext(() => {
    ...
    if (nuxt.options.debug && nuxt.options.debug.hooks && (nuxt.options.debug.hooks === true || nuxt.options.debug.hooks.server)) {
      (0, _hookable.createDebugger)(nuxt.hooks, { tag: "nuxt" });
    }
  });
  if (opts.ready !== false) {
    await nuxt.ready();
...
async function checkDependencyVersion(name, nuxtVersion) {
  const path = (0, _exsolve.resolveModulePath)(name, { try: true });
  if (!path) {
    return;
  }
  const { version: version2 } = await (0, _pkgTypes.readPackageJSON)(path);
  if (version2 && (0, _semver.gt)(nuxtVersion, version2)) {
...
function deduplicateArray(maybeArray) {
  ...
  for (const item of maybeArray) {
    const _hash = (0, _ohash.hash)(item);
    if (!hashes.has(_hash)) {
      hashes.add(_hash);
      fresh.push(item);
    }
...
const appComponentTemplate = {
  ...
  getContents: (ctx) => (0, _knitwork.genExport)(ctx.app.mainComponent, ["default"])
};
const rootComponentTemplate = {
  ...
  getContents: (ctx) => (ctx.nuxt.options.dev ? "import '#build/components.islands.mjs';\n" : "") + (0, _knitwork.genExport)(ctx.app.rootComponent, ["default"])
};
const errorComponentTemplate = {
  filename: "error-component.mjs",
  getContents: (ctx) => (0, _knitwork.genExport)(ctx.app.errorComponent, ["default"])
};
const testComponentWrapperTemplate = {
  filename: "test-component-wrapper.mjs",
  getContents: (ctx) => (0, _knitwork.genExport)((0, _pathe.resolve)(ctx.nuxt.options.appDir, "components/test-component-wrapper"), ["default"])
};
const cssTemplate = {
  filename: "css.mjs",
  getContents: (ctx) => ctx.nuxt.options.css.map((i) => (0, _knitwork.genImport)(i)).join("\n")
};
...
const clientPluginTemplate = {
  ...
  async getContents(ctx) {
    ...
    for (const plugin of clientPlugins) {
      const path = (0, _pathe.relative)(ctx.nuxt.options.rootDir, plugin.src);
      const variable = (0, _knitwork.genSafeVariableName)((0, _utils2.filename)(plugin.src) || path).replace(PLUGIN_TEMPLATE_RE, "_") + "_" + (0, _ohash.hash)(path).replace(/-/g, "_");
      exports.push(variable);
      imports.push((0, _knitwork.genImport)(plugin.src, variable));
    }
    return [
    ...imports,
    `export default ${(0, _knitwork.genArrayFromRaw)(exports)}`].
    join("\n");
  }
};
...
const pluginsDeclaration = {
  ...
  getContents: async ({ nuxt, app }) => {
    const EXTENSION_RE2 = new RegExp(`(?<=\\w)(${nuxt.options.extensions.map((e) => (0, _escapeStringRegexp.default)(e)).join("|")})$`, "g");
    const typesDir = (0, _pathe.join)(nuxt.options.buildDir, "types");
    const tsImports = [];
    const pluginNames = [];
    function exists(path) {
      return app.templates.some((t) => t.write && path === t.dst) || (0, _nodeFs.existsSync)(path);
    }
    for (const plugin of await annotatePlugins(nuxt, app.plugins)) {
      if (plugin.name) {
        pluginNames.push(`'${plugin.name}'`);
      }
      const pluginPath = (0, _pathe.resolve)(typesDir, plugin.src);
      const relativePath = (0, _pathe.relative)(typesDir, pluginPath);
...
type NuxtAppInjections = 
  ${tsImports.map((p) => `InjectionType<typeof ${(0, _knitwork.genDynamicImport)(p, { wrapper: false })}>`).join(" &\n  ")}
...
const schemaTemplate = {
  ...
  getContents: async ({ nuxt }) => {
    const relativeRoot = (0, _pathe.relative)((0, _pathe.resolve)(nuxt.options.buildDir, "types"), nuxt.options.rootDir);
    const getImportName = (name) => (name[0] === "." ? "./" + (0, _pathe.join)(relativeRoot, name) : name).replace(IMPORT_NAME_RE, "");
    const modules = [];
    for (const m of nuxt.options._installedModules) {
      ...
      modules.push([(0, _knitwork.genString)(m.meta.configKey), getImportName(m.entryPath || m.meta.name), m]);
    }
    ...
    ...modules.flatMap(([configKey, importName, mod]) => {
      ...
      `     */`,
      `    [${configKey}]${options.unresolved ? "?" : ""}: typeof ${(0, _knitwork.genDynamicImport)(importName, { wrapper: false })}.default extends NuxtModule<infer O> ? ${options.unresolved ? "Partial<O>" : "O"} : Record<string, any>`];

    }),
    modules.length > 0 && options.unresolved ? `    modules?: (undefined | null | false | NuxtModule<any> | string | [NuxtModule | string, Record<string, any>] | ${modules.map(([configKey, importName, mod]) => `[${(0, _knitwork.genString)(mod.meta?.rawPath || importName)}, Exclude<NuxtConfig[${configKey}], boolean>]`).join(" | ")})[],` : ""].
    filter(Boolean);
    return [
    ...
    "  }",
    (0, _untyped.generateTypes)(
      await (0, _untyped.resolveSchema)(privateRuntimeConfig),
      {
    ...
    (0, _untyped.generateTypes)(
      await (0, _untyped.resolveSchema)(nuxt.options.runtimeConfig.public),
      {
...
const layoutTemplate = {
  ...
  getContents({ app }) {
    const layoutsObject = (0, _knitwork.genObjectFromRawEntries)(Object.values(app.layouts).map(({ name, file }) => {
      return [name, `defineAsyncComponent(${(0, _knitwork.genDynamicImport)(file, { interopDefault: true })})`];
    }));
    return [
    `import { defineAsyncComponent } from 'vue'`,
...
const middlewareTemplate = {
  ...
  getContents({ app }) {
    ...
    const namedMiddleware = app.middleware.filter((mw) => !mw.global);
    const namedMiddlewareObject = (0, _knitwork.genObjectFromRawEntries)(namedMiddleware.map((mw) => [mw.name, (0, _knitwork.genDynamicImport)(mw.path)]));
    return [
    ...globalMiddleware.map((mw) => (0, _knitwork.genImport)(mw.path, (0, _knitwork.genSafeVariableName)(mw.name))),
    `export const globalMiddleware = ${(0, _knitwork.genArrayFromRaw)(globalMiddleware.map((mw) => (0, _knitwork.genSafeVariableName)(mw.name)))}`,
...
const nitroSchemaTemplate = {
  ...
  async getContents({ nuxt }) {
    ...
    const sourceDir = (0, _pathe.join)(nuxt.options.buildDir, "types");
    const lines = [
    ...references.map((ref) => {
      if ("path" in ref && (0, _pathe.isAbsolute)(ref.path)) {
        ref.path = (0, _pathe.relative)(sourceDir, ref.path);
      }
...
const appConfigDeclarationTemplate = {
  ...
  getContents({ app, nuxt }) {
    const typesDir = (0, _pathe.join)(nuxt.options.buildDir, "types");
    const configPaths = app.configs.map((path) => (0, _pathe.relative)(typesDir, path).replace(EXTENSION_RE, ""));
    return `
...
const nuxtConfigTemplate = {
  ...
  getContents: (ctx) => {
    ...
    return [
    ...Object.entries(ctx.nuxt.options.app).map(([k, v]) => `export const ${(0, _scule.camelCase)("app-" + k)} = ${JSON.stringify(v)}`),
...
const buildTypeTemplate = {
  ...
  getContents({ app }) {
    ...
    for (const file of app.templates) {
      ...
      declarations += "declare module " + JSON.stringify((0, _pathe.join)("#build", file.filename)) + ";\n";
    }
    return declarations;
  }
};
...
async function generateApp(nuxt, app, options = {}) {
  ...
  app.templates = app.templates.map((tmpl) => (0, _kit.normalizeTemplate)(tmpl, nuxt.options.buildDir));
  ...
  const templateContext = { utils: _kit.templateUtils, nuxt, app };
  const compileTemplate$1 = nuxt.options.experimental.compileTemplate ? _kit.compileTemplate : futureCompileTemplate;
  const writes = [];
  const dirs = /* @__PURE__ */new Set();
  const changedTemplates = [];
  const FORWARD_SLASH_RE = /\//g;
  async function processTemplate(template) {
    const fullPath = template.dst || (0, _pathe.resolve)(nuxt.options.buildDir, template.filename);
    ...
    if (template.modified && template.write) {
      dirs.add((0, _pathe.dirname)(fullPath));
      writes.push(() => (0, _nodeFs.writeFileSync)(fullPath, contents, "utf8"));
    }
  }
...
async function futureCompileTemplate(template, ctx) {
  ...
  if (template.src) {
    try {
      return await _nodeFs.promises.readFile(template.src, "utf-8");
    } catch (err) {
...
async function resolveApp(nuxt, app) {
  app.mainComponent ||= await (0, _kit.findPath)(
    nuxt.options._layers.flatMap((layer) => [
    (0, _pathe.join)(layer.config.srcDir, "App"),
    (0, _pathe.join)(layer.config.srcDir, "app")]
    )
  );
  app.mainComponent ||= (0, _pathe.resolve)(nuxt.options.appDir, "components/welcome.vue");
  app.rootComponent ||= await (0, _kit.findPath)(["~/app.root", (0, _pathe.resolve)(nuxt.options.appDir, "components/nuxt-root.vue")]);
  app.errorComponent ||= (await (0, _kit.findPath)(
    nuxt.options._layers.map((layer) => (0, _pathe.join)(layer.config.srcDir, "error"))
  )) ?? (0, _pathe.resolve)(nuxt.options.appDir, "components/nuxt-error-page.vue");
  ...
  for (const config of layerConfigs) {
    ...
    const layoutFiles = await (0, _kit.resolveFiles)(config.srcDir, `${layoutDir}/**/*{${nuxt.options.extensions.join(",")}}`);
    for (const file of layoutFiles) {
      const name = getNameFromPath(file, (0, _pathe.resolve)(config.srcDir, layoutDir));
      if (!name) {
        logger.warn(`No layout name could be resolved for \`~/${(0, _pathe.relative)(nuxt.options.srcDir, file)}\`. Bear in mind that \`index\` is ignored for the purpose of creating a layout name.`);
        continue;
      }
  ...
  for (const config of reversedConfigs) {
    ...
    const middlewareFiles = await (0, _kit.resolveFiles)(config.srcDir, [
    ...
    for (const file of middlewareFiles) {
      ...
      if (!name) {
        logger.warn(`No middleware name could be resolved for \`~/${(0, _pathe.relative)(nuxt.options.srcDir, file)}\`. Bear in mind that \`index\` is ignored for the purpose of creating a middleware name.`);
        continue;
      }
  ...
  for (const config of reversedConfigs) {
    ...
    app.plugins.push(...[
    ...(config.plugins || []),
    ...(config.srcDir ? await (0, _kit.resolveFiles)(config.srcDir, [
    `${pluginDir}/*{${nuxt.options.extensions.join(",")}}`,
    `${pluginDir}/*/index{${nuxt.options.extensions.join(",")}}`]
    ) : [])].
    map((plugin) => (0, _kit.normalizePlugin)(plugin)));
  }
  for (const p of [...nuxt.options.plugins].reverse()) {
    const plugin = (0, _kit.normalizePlugin)(p);
  ...
  app.configs = [];
  for (const config of layerConfigs) {
    const appConfigPath = await (0, _kit.findPath)((0, _pathe.resolve)(config.srcDir, "app.config"));
    if (appConfigPath) {
...
function resolvePaths(nuxt, items, key) {
  return Promise.all(items.map(async (item) => {
    ...
    return {
      ...item,
      [key]: await (0, _kit.resolvePath)(item[key], {
...
async function annotatePlugins(nuxt, plugins) {
  ...
  for (const plugin of plugins) {
    try {
      const code = plugin.src in nuxt.vfs ? nuxt.vfs[plugin.src] : await _nodeFs.promises.readFile(plugin.src, "utf-8");
      _plugins.push({
        ...(await extractMetadata(code, IS_TSX.test(plugin.src) ? "tsx" : "ts")),
        ...plugin
      });
    } catch (e) {
      const relativePluginSrc = (0, _pathe.relative)(nuxt.options.rootDir, plugin.src);
...
async function checkAndWarnAboutConfigFileExistence(options) {
  ...
  const configFile = await (0, _kit.findPath)(fileName, { extensions }).catch(() => null);
  if (configFile) {
    return createWarningMessage((0, _pathe.basename)(configFile));
  }
}
...
async function getVueHash(nuxt) {
  ...
  const { hash: hash2 } = await getHashes(nuxt, {
    ...
    patterns: (layer) => [
    (0, _pathe.join)((0, _pathe.relative)(layer.cwd, layer.config.srcDir), "**"),
    `!${(0, _pathe.relative)(layer.cwd, layer.config.serverDir || (0, _pathe.join)(layer.cwd, "server"))}/**`,
    `!${(0, _pathe.relative)(layer.cwd, (0, _nodePath.resolve)(layer.config.srcDir || layer.cwd, layer.config.dir?.public || "public"))}/**`,
    `!${(0, _pathe.relative)(layer.cwd, (0, _nodePath.resolve)(layer.config.srcDir || layer.cwd, layer.config.dir?.static || "public"))}/**`,
    "!node_modules/**",
    "!nuxt.config.*"],
  ...
  const cacheFile = (0, _pathe.join)(nuxt.options.workspaceDir, "node_modules/.cache/nuxt/builds", id, hash2 + ".tar");
  return {
    hash: hash2,
    async collectCache() {
      ...
      _consola.consola.success(`Cached Vue client and server builds in \`${elapsed}ms\`.`);
    },
    async restoreCache() {
      const start = Date.now();
      ...
      const elapsed = Date.now() - start;
      if (res) {
        _consola.consola.success(`Restored Vue client and server builds from cache in \`${elapsed}ms\`.`);
      }
      return res;
    }
  };
...
async function cleanupCaches(nuxt) {
  const start = Date.now();
  const caches = await (0, _tinyglobby.glob)(["*/*.tar"], {
    cwd: (0, _pathe.join)(nuxt.options.workspaceDir, "node_modules/.cache/nuxt/builds"),
    absolute: true
  });
  if (caches.length >= 10) {
    const cachesWithMeta = await Promise.all(caches.map(async (cache) => {
      return [cache, await (0, _promises.stat)(cache).then((r) => r.mtime.getTime()).catch(() => 0)];
    }));
    cachesWithMeta.sort((a, b) => a[1] - b[1]);
    for (const [cache] of cachesWithMeta.slice(0, cachesWithMeta.length - 10)) {
      await (0, _promises.unlink)(cache);
    }
    const elapsed = Date.now() - start;
    _consola.consola.success(`Cleaned up old build caches in \`${elapsed}ms\`.`);
  }
}
async function getHashes(nuxt, options) {
  ...
  const res = nuxt[`_${options.id}BuildHash`] = {
    hash: (0, _ohash.hash)(hashSources),
    sources: hashSources
  };
  const elapsed = Date.now() - start;
  _consola.consola.debug(`Computed \`${options.id}\` build hash in \`${elapsed}ms\`.`);
  return res;
}
async function readFilesRecursive(dir, opts) {
  ...
  const files = await (0, _tinyglobby.glob)(opts.patterns, { cwd: dir });
  const fileEntries = await Promise.all(files.map(async (fileName) => {
    if (!opts.shouldIgnore?.(fileName)) {
      ...
      return {
        ...file,
        name: (0, _pathe.relative)(opts.cwd, (0, _pathe.join)(dir, file.name))
      };
    }
  }));
...
async function readFileWithMeta(dir, fileName, count = 0) {
  let fd = void 0;
  try {
    fd = await (0, _promises.open)((0, _nodePath.resolve)(dir, fileName));
...
async function restoreCache(cwd, cacheFile) {
  if (!(0, _nodeFs.existsSync)(cacheFile)) {
    return false;
  }
  const files = (0, _nanotar.parseTar)(await (0, _promises.readFile)(cacheFile));
  for (const file of files) {
    let fd = void 0;
    try {
      const filePath = (0, _nodePath.resolve)(cwd, file.name);
      await (0, _promises.mkdir)((0, _pathe.dirname)(filePath), { recursive: true });
      fd = await (0, _promises.open)(filePath, "w");
      ...
      if (stats?.isFile() && stats.size) {
        ...
        if (stats.mtime.getTime() >= lastModified) {
          _consola.consola.debug(`Skipping \`${file.name}\` (up to date or newer than cache)`);
          continue;
        }
      }
...
async function writeCache(cwd, sources, cacheFile) {
  ...
  const tarData = (0, _nanotar.createTar)(fileEntries);
  await (0, _promises.mkdir)((0, _pathe.dirname)(cacheFile), { recursive: true });
  await (0, _promises.writeFile)(cacheFile, tarData);
}

async function build(nuxt) {
  const app = createApp(nuxt);
  nuxt.apps.default = app;
  const generateApp$1 = (0, _perfectDebounce.debounce)(() => generateApp(nuxt, app), void 0, { leading: true });
  await generateApp$1();
  if (nuxt.options.dev) {
    watch(nuxt);
    nuxt.hook("builder:watch", async (event, relativePath) => {
      if (event === "add" || event === "unlink") {
        const path = (0, _pathe.resolve)(nuxt.options.srcDir, relativePath);
        for (const layer of nuxt.options._layers) {
          const relativePath2 = (0, _pathe.relative)(layer.config.srcDir || layer.cwd, path);
...
function createWatcher() {
  const nuxt = (0, _kit.useNuxt)();
  const isIgnored2 = (0, _kit.createIsIgnored)(nuxt);
  const watcher = (0, _chokidar.watch)(nuxt.options._layers.map((i) => i.config.srcDir).filter(Boolean), {
    ...nuxt.options.watchers.chokidar,
    ignoreInitial: true,
  ...
  for (const pattern of nuxt.options.watch) {
    ...
    const path = (0, _pathe.resolve)(nuxt.options.srcDir, pattern);
  ...
  watcher.on("all", (event, path) => {
    ...
    nuxt.callHook("builder:watch", event, nuxt.options.experimental.relativeWatchPaths ? (0, _pathe.normalize)((0, _pathe.relative)(nuxt.options.srcDir, path)) : (0, _pathe.normalize)(path));
  });
  nuxt.hook("close", () => watcher?.close());
}
function createGranularWatcher() {
  ...
  for (const dir of pathsToWatch) {
    pending++;
    const watcher = (0, _chokidar.watch)(dir, { ...nuxt.options.watchers.chokidar, ignoreInitial: false, depth: 0, ignored: [isIgnored2, /[\\/]node_modules[\\/]/] });
    const watchers = {};
    watcher.on("all", (event, path) => {
      if (event === "all" || event === "ready" || event === "error" || event === "raw") {
        return;
      ...
      if (!pending) {
        nuxt.callHook("builder:watch", event, nuxt.options.experimental.relativeWatchPaths ? (0, _pathe.relative)(nuxt.options.srcDir, path) : path);
      }
      ...
      if (event === "addDir" && path !== dir && !ignoredDirs.has(path) && !pathsToWatch.includes(path) && !(path in watchers) && !isIgnored2(path)) {
        const pathWatcher = watchers[path] = (0, _chokidar.watch)(path, { ...nuxt.options.watchers.chokidar, ignored: [isIgnored2] });
        pathWatcher.on("all", (event2, p) => {
          if (event2 === "all" || event2 === "ready" || event2 === "error" || event2 === "raw") {
            return;
          }
          nuxt.callHook("builder:watch", event2, nuxt.options.experimental.relativeWatchPaths ? (0, _pathe.normalize)((0, _pathe.relative)(nuxt.options.srcDir, p)) : (0, _pathe.normalize)(p));
        });
...
async function createParcelWatcher() {
  ...
  try {
    const { subscribe } = await (0, _kit.importModule)("@parcel/watcher", { url: [nuxt.options.rootDir, ...nuxt.options.modulesDir].map((d) => (0, _kit.directoryToURL)(d)) });
    for (const layer of nuxt.options._layers) {
      ...
      const watcher = subscribe(layer.config.srcDir, (err, events) => {
        ...
        for (const event of events) {
          if ((0, _kit.isIgnored)(event.path)) {
            continue;
          }
          nuxt.callHook("builder:watch", watchEvents[event.type], nuxt.options.experimental.relativeWatchPaths ? (0, _pathe.normalize)((0, _pathe.relative)(nuxt.options.srcDir, event.path)) : (0, _pathe.normalize)(event.path));
        }
      }, {
        ignore: [
        ...nuxt.options.ignore,
...
async function loadBuilder(nuxt, builder) {
  try {
    return await (0, _kit.importModule)(builder, { url: [(0, _kit.directoryToURL)(nuxt.options.rootDir), new URL("file:///C:/Users/admin/Documents/Front_End_Web_Hotel_Nhom_2/node_modules/nuxt/dist/shared/nuxt.iYoBCR2R.mjs")] });
  } catch {
