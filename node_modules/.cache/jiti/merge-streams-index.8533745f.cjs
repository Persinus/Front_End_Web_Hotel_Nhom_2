"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.default = mergeStreams;var _nodeEvents = require("node:events");
...
const onMergedStreamEnd = async (passThroughStream, { signal }) => {
  await (0, _promises.finished)(passThroughStream, { signal, cleanup: true });
};

const onInputStreamsUnpipe = async (passThroughStream, streams, { signal }) => {
  for await (const [unpipedStream] of (0, _nodeEvents.on)(passThroughStream, 'unpipe', { signal })) {
...
const onInputStreamEnd = async ({ passThroughStream, stream, streams, ended, aborted, controller: { signal } }) => {
  try {
    await (0, _promises.finished)(stream, { signal, cleanup: true, readable: true, writable: false });
...
const onInputStreamUnpipe = async ({ stream, streams, ended, aborted, controller: { signal } }) => {
  await (0, _nodeEvents.once)(stream, unpipeEvent, { signal });
...
const PASSTHROUGH_LISTENERS_PER_STREAM = 1; /* v9-4f4c884d1d87ce15 */
