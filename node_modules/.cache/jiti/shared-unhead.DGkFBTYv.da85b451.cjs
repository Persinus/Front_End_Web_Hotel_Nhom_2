"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = createUnhead;exports.c = createHeadCore;var _hookable = require("hookable");
var _unheadCp0iF6eN = require("./unhead.Cp0iF6eN.mjs");
var _unheadDZbvapt = require("./unhead.DZbvapt-.mjs");
var _unheadYem5I2v_ = require("./unhead.yem5I2v_.mjs");
  ...
  const head = {
    ...
    async resolveTags() {
      ...
      while (normalizeQueue.length) {
        ...
        if (e) {
          const normalizeCtx = {
            tags: (0, _unheadCp0iF6eN.n)(e.input, resolvedOptions.propResolvers || []).map((t) => Object.assign(t, e.options)),
            entry: e
          };
          await hooks.callHook("entries:normalize", normalizeCtx);
          e._tags = normalizeCtx.tags.map((t, i2) => {
            t._w = (0, _unheadDZbvapt.t)(head, t);
            t._p = (e._i << 10) + i2;
            t._d = (0, _unheadCp0iF6eN.d)(t);
            return t;
          });
        }
      }
      let hasFlatMeta = false;
      ctx.entries.flatMap((e) => (e._tags || []).map((t) => ({ ...t, props: { ...t.props } }))).sort(_unheadDZbvapt.s).reduce((acc, next) => {
        const k = String(next._d || next._p);
        if (!acc.has(k))
        return acc.set(k, next);
        const prev = acc.get(k);
        const strategy = next?.tagDuplicateStrategy || (_unheadYem5I2v_.c.has(next.tag) ? "merge" : null) || (next.key && next.key === prev.key ? "merge" : null);
        if (strategy === "merge") {
        ...
        } else if (next._p >> 10 === prev._p >> 10 && (0, _unheadCp0iF6eN.i)(next._d)) {
      ...
      if (hasFlatMeta) {
        ctx.tags = ctx.tags.flat().sort(_unheadDZbvapt.s);
      }
