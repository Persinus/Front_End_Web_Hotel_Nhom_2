"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.addBuildPlugin = addBuildPlugin;exports.addComponent = addComponent;exports.addComponentsDir = addComponentsDir;exports.addDevServerHandler = addDevServerHandler;exports.addImports = addImports;exports.addImportsDir = addImportsDir;exports.addImportsSources = addImportsSources;exports.addLayout = addLayout;exports.addPlugin = addPlugin;exports.addPluginTemplate = addPluginTemplate;exports.addPrerenderRoutes = addPrerenderRoutes;exports.addRouteMiddleware = addRouteMiddleware;exports.addRspackPlugin = addRspackPlugin;exports.addServerHandler = addServerHandler;exports.addServerImports = addServerImports;exports.addServerImportsDir = addServerImportsDir;exports.addServerPlugin = addServerPlugin;exports.addServerScanDir = addServerScanDir;exports.addServerTemplate = addServerTemplate;exports.addTemplate = addTemplate;exports.addTypeTemplate = addTypeTemplate;exports.addVitePlugin = addVitePlugin;exports.addWebpackPlugin = addWebpackPlugin;exports.assertNuxtCompatibility = assertNuxtCompatibility;exports.buildNuxt = buildNuxt;exports.checkNuxtCompatibility = checkNuxtCompatibility;exports.compileTemplate = compileTemplate;exports.createIsIgnored = createIsIgnored;exports.createResolver = createResolver;exports.defineNuxtModule = defineNuxtModule;exports.directoryToURL = directoryToURL;exports.extendNuxtSchema = extendNuxtSchema;exports.extendPages = extendPages;exports.extendRouteRules = extendRouteRules;exports.extendRspackConfig = void 0;exports.extendViteConfig = extendViteConfig;exports.extendWebpackConfig = void 0;exports.findPath = findPath;exports.getDirectory = getDirectory;exports.getNuxtCtx = void 0;exports.getNuxtModuleVersion = getNuxtModuleVersion;exports.getNuxtVersion = getNuxtVersion;exports.hasNuxtCompatibility = hasNuxtCompatibility;exports.hasNuxtModule = hasNuxtModule;exports.hasNuxtModuleCompatibility = hasNuxtModuleCompatibility;exports.importModule = importModule;exports.installModule = installModule;exports.isIgnored = isIgnored;exports.isNuxt2 = isNuxt2;exports.isNuxt3 = isNuxt3;exports.isNuxtMajorVersion = isNuxtMajorVersion;exports.loadNuxt = loadNuxt;exports.loadNuxtConfig = loadNuxtConfig;exports.loadNuxtModuleInstance = loadNuxtModuleInstance;exports.normalizeModuleTranspilePath = exports.logger = void 0;exports.normalizePlugin = normalizePlugin;exports.normalizeSemanticVersion = normalizeSemanticVersion;exports.normalizeTemplate = normalizeTemplate;exports.nuxtCtx = void 0;exports.requireModule = requireModule;exports.resolveAlias = resolveAlias;exports.resolveFiles = resolveFiles;exports.resolveIgnorePatterns = resolveIgnorePatterns;exports.resolveModule = resolveModule;exports.resolveNuxtModule = resolveNuxtModule;exports.resolvePath = resolvePath;exports.runWithNuxtContext = runWithNuxtContext;exports.templateUtils = void 0;exports.tryImportModule = tryImportModule;exports.tryRequireModule = tryRequireModule;exports.tryResolveModule = tryResolveModule;exports.tryUseNuxt = tryUseNuxt;exports.updateRuntimeConfig = updateRuntimeConfig;exports.updateTemplates = updateTemplates;exports.useLogger = useLogger;exports.useNitro = useNitro;exports.useNuxt = useNuxt;exports.useRuntimeConfig = useRuntimeConfig;exports.writeTypes = writeTypes;var _nodeFs = require("node:fs");
...
var _satisfies = _interopRequireDefault(require("semver/functions/satisfies.js"));
...
var _utils = require("pathe/utils");
  ...
  if (constraints.builder && typeof nuxt.options.builder === "string") {
    ...
    if (currentBuilder in constraints.builder) {
      ...
      if (constraint === false) {
        ...
        for (const parent of [nuxt.options.rootDir, nuxt.options.workspaceDir, "file:///C:/Users/admin/Documents/Front_End_Web_Hotel_Nhom_2/node_modules/@nuxt/kit/dist/index.mjs"]) {
...
const distDirURL = new URL(".", "file:///C:/Users/admin/Documents/Front_End_Web_Hotel_Nhom_2/node_modules/@nuxt/kit/dist/index.mjs");

async function compileTemplate(template$1, ctx) {
  const data = { ...ctx, options: template$1.options };
  if (template$1.src) {
    try {
      const srcContents = await _nodeFs.promises.readFile(template$1.src, "utf-8");
...
const importSources = (sources, { lazy = false } = {}) => {
  return toArray(sources).map((src) => {
    const safeVariableName = (0, _knitwork.genSafeVariableName)(src);
    if (lazy) {
      return `const ${safeVariableName} = ${(0, _knitwork.genDynamicImport)(src, { comment: `webpackChunkName: ${JSON.stringify(src)}` })}`;
    }
    return (0, _knitwork.genImport)(src, safeVariableName);
  }).join("\n");
};
const importName = _knitwork.genSafeVariableName;
const templateUtils = exports.templateUtils = { serialize, importName, importSources };
...
function nuxt2Shims(nuxt) {
  ...
  nuxt.hook("build:templates", async (templates) => {
    ...
    for await (const template of virtualTemplates) {
      ...
      await _nodeFs.promises.mkdir((0, _pathe.dirname)(template.dst), { recursive: true });
      await _nodeFs.promises.writeFile(template.dst, contents);
    }
  });
}
...
function tryResolveModule(id, url = "file:///C:/Users/admin/Documents/Front_End_Web_Hotel_Nhom_2/node_modules/@nuxt/kit/dist/index.mjs") {
...
function resolveModule(id, options) {
  return (0, _exsolve.resolveModulePath)(id, {
    from: options?.url ?? options?.paths ?? ["file:///C:/Users/admin/Documents/Front_End_Web_Hotel_Nhom_2/node_modules/@nuxt/kit/dist/index.mjs"],
...
function requireModule(id, opts) {
  ...
  const resolvedPath = resolveModule(id, opts);
  const jiti = (0, _jiti.createJiti)("file:///C:/Users/admin/Documents/Front_End_Web_Hotel_Nhom_2/node_modules/@nuxt/kit/dist/index.mjs", {
...
async function existsSensitive(path) {
  const dirFiles = await _nodeFs.promises.readdir((0, _pathe.dirname)(path)).catch(() => null);
  return dirFiles && dirFiles.includes((0, _pathe.basename)(path));
}
...
async function resolveFiles(path, pattern, opts = {}) {
  const files = [];
  for (const file of await (0, _globby.globby)(pattern, { cwd: path, followSymbolicLinks: opts.followSymbolicLinks ?? true })) {
    const p = (0, _pathe.resolve)(path, file);
    if (!isIgnored(p)) {
      files.push(p);
    }
  }
...
async function installModule(moduleToInstall, inlineOptions, nuxt = useNuxt()) {
  ...
  if (typeof modulePath === "string") {
    ...
    if (directory !== moduleToInstall && !localLayerModuleDirs.some((dir) => directory.startsWith(dir))) {
      nuxt.options.modulesDir.push((0, _pathe.resolve)(directory, "node_modules"));
    }
  }
  ...
  nuxt.options._installedModules.push({
    meta: (0, _defu.defu)(await nuxtModule.getMeta?.(), buildTimeModuleMeta),
    module: nuxtModule,
    timings: res.timings,
    entryPath
  });
...
async function loadNuxtConfig(opts) {
  const localLayers = await (0, _globby.globby)("layers/*", { onlyDirectories: true, cwd: opts.cwd || process.cwd() });
  ...
  if (!opts.overrides?._prepare && !nuxtConfig.dev && !nuxtConfig.buildDir && nuxtConfig.future?.compatibilityVersion === 4 && (0, _nodeFs.existsSync)(defaultBuildDir)) {
  ...
  for (const layer of layers) {
    ...
    if (layer.cwd && cwd && localLayers.includes((0, _pathe.relative)(cwd, layer.cwd))) {
      layer.meta ||= {};
...
async function loadNuxtSchema(cwd) {
  ...
  if (nuxtPath) {
    urls.unshift((0, _nodeUrl.pathToFileURL)(nuxtPath));
  }
...
async function loadNuxt(opts) {
  ...
  const resolvedPath = ["nuxt-nightly", "nuxt3", "nuxt", "nuxt-edge"].map((pkg2) => (0, _exsolve.resolveModulePath)(pkg2, { try: true, from: [directoryToURL(opts.cwd)] })).filter((p) => !!p).sort((a, b) => b.length - a.length)[0];
  if (!resolvedPath) {
    throw new Error(`Cannot find any nuxt version from ${opts.cwd}`);
  }
  const pkg = await (0, _pkgTypes.readPackageJSON)(resolvedPath);
  const majorVersion = pkg.version ? Number.parseInt(pkg.version.split(".")[0]) : "";
  if (majorVersion && majorVersion >= 3) {
    const { loadNuxt: loadNuxt3 } = await ((specifier) => new Promise((r) => r(`${specifier}`)).then((s) => jitiImport(s).then((m) => _interopRequireWildcard(m))))((0, _nodeUrl.pathToFileURL)(resolvedPath).href).then((r) => (0, _mlly.interopDefault)(r));
...
function normalizeTemplate(template, buildDir) {
  ...
  if (template.src) {
    ...
    if (!template.filename) {
      ...
      template.filename = template.fileName || `${(0, _pathe.basename)(srcPath.dir)}.${srcPath.name}.${(0, _ohash.hash)(template.src)}${srcPath.ext}`;
    }
  }
...
async function _generateTypes(nuxt) {
  ...
  (0, _pathe.join)(relativeRootDir, "**/*")]
  );
  if (nuxt.options.srcDir !== nuxt.options.rootDir) {
    include.add((0, _pathe.join)((0, _pathe.relative)(nuxt.options.buildDir, nuxt.options.srcDir), "**/*"));
  }
  ...
  for (const layer of nuxt.options._layers) {
    ...
    if (!srcOrCwd.startsWith(rootDirWithSlash) || srcOrCwd.includes("node_modules")) {
      include.add((0, _pathe.join)((0, _pathe.relative)(nuxt.options.buildDir, srcOrCwd), "**/*"));
    }
  }
  ...
  for (const dir of [...nuxt.options.modulesDir].sort()) {
    const withSlash = (0, _ufo.withTrailingSlash)(dir);
...
async function writeTypes(nuxt) {
  ...
  async function writeFile() {
    ...
    const tsConfigPath = (0, _pathe.resolve)(nuxt.options.buildDir, "tsconfig.json");
    await _nodeFs.promises.mkdir(nuxt.options.buildDir, { recursive: true });
    await _nodeFs.promises.writeFile(tsConfigPath, GeneratedBy + "\n" + JSON.stringify(tsConfig, null, 2));
    const declarationPath = (0, _pathe.resolve)(nuxt.options.buildDir, "nuxt.d.ts");
    await _nodeFs.promises.writeFile(declarationPath, GeneratedBy + "\n" + declaration);
  }
...
function addLayout(template, name) {
  ...
  nuxt.hook("app:templates", (app) => {
    if (layoutName in app.layouts) {
      const relativePath = (0, _pathe.relative)(nuxt.options.srcDir, app.layouts[layoutName].file);
      return logger.warn(
