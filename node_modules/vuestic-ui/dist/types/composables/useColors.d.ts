import type { ColorVariables, CssColor } from 'vuestic-ui/dist/types/services/color/index.js';
import { type ColorInput } from 'vuestic-ui/dist/types/services/color/utils.js';
/**
 * You can add these props to any component by destructuring them inside props option.
 * @example
 * props: { ...useColorProps, componentsOwnProp, etc. }
 * It's better to add props at the beginning to make sure that component own props will be used instead in case of collision.
 */
export declare const useColorProps: {
    color: {
        type: StringConstructor;
        default: string;
    };
};
export declare const useColors: () => {
    colors: ColorVariables;
    currentPresetName: import("vue").WritableComputedRef<string>;
    presets: import("vue").ComputedRef<{
        [presetName: string]: ColorVariables;
        light: ColorVariables;
        dark: ColorVariables;
    }>;
    applyPreset: (presetName: string) => boolean | undefined;
    setColors: (colors: Partial<ColorVariables>) => void;
    getColors: () => ColorVariables;
    getColor: (prop?: string, defaultColor?: string, preferVariables?: boolean) => CssColor;
    getComputedColor: (color: string) => import("vue").WritableComputedRef<string>;
    getBoxShadowColor: (color: string, opacity?: number) => string;
    getBoxShadowColorFromBg: (background: string, opacity?: number) => string;
    getHoverColor: (color: string, opacity?: number) => string;
    getFocusColor: (color: string, opacity?: number) => string;
    getGradientBackground: (color: string) => string;
    getTextColor: (color: ColorInput, darkColor?: string, lightColor?: string) => string;
    shiftHSLAColor: (color: string | import("vuestic-ui/dist/types/utils/color.js").RGBObject | import("vuestic-ui/dist/types/utils/color.js").HSLObject, shift: {
        h?: number | undefined;
        s?: number | undefined;
        l?: number | undefined;
        a?: number | undefined;
    }) => string;
    setHSLAColor: (color: string | import("vuestic-ui/dist/types/utils/color.js").RGBObject | import("vuestic-ui/dist/types/utils/color.js").HSLObject, shift: {
        h?: number | undefined;
        s?: number | undefined;
        l?: number | undefined;
        a?: number | undefined;
    }) => string;
    colorsToCSSVariable: (colors: {
        [colorName: string]: string | undefined;
    }, prefix?: string) => Record<string, any>;
    colorToRgba: (color: string, opacity: number) => string;
    getStateMaskGradientBackground: (color: string, maskColor: string, maskOpacity: number) => string;
};
export * from 'vuestic-ui/dist/types/services/color/utils.js';
export * from 'vuestic-ui/dist/types/services/color/index.js';
